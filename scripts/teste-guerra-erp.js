...
async function gerarRelatorioConsolidado(resultados) {
  ...
  const relatorioMd = `# Relatório de Teste de Guerra ERP Completo
...
`;
  ...
  await fs.writeFile('RELATORIO_TESTE_GUERRA_ERP.md', relatorioMd);
  console.log('Relatório consolidado salvo em RELATORIO_TESTE_GUERRA_ERP.md');
}

// Função principal
async function main() {
  console.log('=== INICIANDO TESTE DE GUERRA ERP COMPLETO ===');
  ...
  resultados.testeCarga = executarScript('scripts/teste-carga-erp.js');
  
  // Executar teste de idempotência
  resultados.testeIdempotencia = executarScript('scripts/teste-idempotencia.js');
  
  // Executar teste de cache
  resultados.testeCache = executarScript('scripts/teste-cache.js');
  
  // Executar teste de logs
  resultados.testeLogs = executarScript('scripts/teste-logs.js');
  
  // Gerar relatório consolidado
  await gerarRelatorioConsolidado(resultados);
  
  console.log('\n=== TESTE DE GUERRA ERP COMPLETO FINALIZADO ===');
}
