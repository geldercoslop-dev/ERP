  ...
  try {
    ...
    for (const t of expected) {
      ...
      const [rows] = await conn.query(
        "SELECT COUNT(*) AS c FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = ? AND column_name = 'tenant_id'",
        [t]
      );
    ...
    const [tenantsRows] = await conn.query(
      "SELECT COUNT(*) AS c FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'tenants'"
    );

    console.log("MISSING_TENANT_ID=" + missing.join(","));
    console.log("TENANTS_EXISTS=" + (Number(tenantsRows[0].c) > 0 ? "1" : "0"));
