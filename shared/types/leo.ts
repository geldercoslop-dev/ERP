...
export enum LeoEventStatus {
  ATIVO = 'ativo',
  RESOLVIDO = 'resolvido',
  IGNORADO = 'ignorado'
}

export enum LeoEventPriority {
  BAIXA = 'baixa',
  MEDIA = 'media',
  ALTA = 'alta',
  CRITICA = 'critica'
...
export enum LeoCommandOrigin {
  USUARIO = 'usuario',
  SISTEMA = 'sistema',
  AUTOMACAO = 'automacao'
}

export enum LeoInsightType {
  PADRAO = 'padrao',
  ANOMALIA = 'anomalia',
...
export interface ILeoEvents {
  criarEvento(input: unknown): Promise<{ success: boolean; eventId?: number; message: string }>;
  /** @deprecated Use registerEvent */
  registrarEvento?(input: unknown): Promise<{ success: boolean; eventId?: number; message: string }>;
  registerEvent(input: unknown): Promise<LeoEvent>;
  /** @deprecated Use listEvents */
  listarEventos?(filtros?: unknown): Promise<{ success: boolean; eventos?: LeoEvent[]; message: string }>;
  listEvents(filtros?: { tipo?: string; status?: string; usuarioResponsavel?: string; limit?: number }): Promise<LeoEvent[]>;
