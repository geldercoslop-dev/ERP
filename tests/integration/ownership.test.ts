...
import { getDb, getInsertId } from "../../server/db/index.js";
import {
  afterAllWithServiceContext,
  beforeAllWithServiceContext,
  itWithServiceContext,
} from "./service-context-harness.js";
import type { Cliente } from "../../server/db/core.js";
import type { ServiceActor } from "../../server/_core/service-actor.js";
import * as clientesService from "../../server/services/clientes.service.js";
import {
  assertPedidoMutableByActor,
  createPedidoSafe,
  getPedidoById,
  PedidoAccessError,
} from "../../server/services/orders.service.js";
import {
  users,
  vendedores,
  clientes,
...
describe.skipIf(!INTEGRATION_DB_READY)("OWNERSHIP HARDENING - TESTE REAL", () => {
  ...
  it("CLIENTES: Vendedor A NÃO acessa cliente B via service", async () => {
  ...
  it("CLIENTES: Admin acessa cliente A", async () => {
  ...
  it("CLIENTES: Vendedor A não pode editar cliente B", async () => {
    await expect(
  ...
  it("PEDIDOS: Vendedor B acessa pedido de cliente B", async () => {
  ...
  it("PEDIDOS: Admin acessa pedido de cliente A", async () => {
  ...
  it("SEGURANÇA: getClienteById negado retorna null", async () => {
