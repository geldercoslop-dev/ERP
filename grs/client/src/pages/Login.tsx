...
function LoginForm({
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col" style={{ gap: 0, width: "100%" }}>
      ...
      {/* Botão */}
      <button type="submit" disabled={!canSubmit} className="wood-btn w-full font-bold tracking-widest uppercase"
...
export default function Login() {
  ...
  const canSubmit = useMemo(() => !!username.trim() && !!password && !isLoading, [username, password, isLoading]);
  ...
  const formProps = { username, setUsername, password, setPassword, showPass, setShowPass, error, canSubmit, isLoading, onSubmit };
      ...
      <div className="login-desktop relative z-10 w-full max-w-[900px]">
        <div className="relative w-full overflow-hidden rounded-2xl"
          ...
          <img src="/login-grs-leo-v2.png?cb=20260227e" alt="GRS Móveis"
            className="absolute inset-0 h-full w-full object-cover select-none" draggable={false} />
          <div className="absolute flex flex-col"
            style={{ left: "30%", top: "36%", width: "38%", height: "50%", padding: "0 4%" }}>
