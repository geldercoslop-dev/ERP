...
type ProdutoListItem = Record<string, unknown> & {
  ...
  cores?: { corId: number; estoque: number }[];
  variacoes?: Record<string, unknown>[];
};

type GrupoListItem = { id: number; nome: string };
type CorListItem = { id: number; nome: string };

function extractProdutosList(d: unknown): ProdutoListItem[] {
  if (Array.isArray(d)) return d as ProdutoListItem[];
  if (d && typeof d === "object" && d !== null) {
    const o = d as Record<string, unknown>;
    if (Array.isArray(o.items)) return o.items as ProdutoListItem[];
    if (Array.isArray(o.produtos)) return o.produtos as ProdutoListItem[];
  }
  return [];
}

function nomeFromVariacao(v: Record<string, unknown>): string {
  const nome = v.nome;
  if (typeof nome === "string" && nome.length > 0) return nome.toUpperCase();
  if (v.temEspelho === true) return "COM ESPELHO";
  const t = v.tamanho;
  if (t != null && String(t).length > 0) return String(t).toUpperCase();
  return "";
}
 ...
 */
export default function ProdutosConnected() {
  ...
  const produtos = useMemo(() => extractProdutosList(produtosRaw), [produtosRaw]);
  ...
  const produtosFiltrados = useMemo(() => {
    ...
    if (!produtos.length) return [] as ProdutoListItem[];
    if (!term) return [...produtos].sort((a, b) => b.id - a.id);
    return produtos
      .filter((p) => `${String(p.descricao ?? "")} ${String(p.marca ?? "")}`.toLowerCase().includes(term))
      .sort((a, b) => b.id - a.id);
  }, [produtos, busca]);
  ...
  function carregarParaEdicao(p: ProdutoListItem) {
    setEditingId(p.id);
    setFormData((prev) => ({
      ...prev,
    ...
    if (Array.isArray(p.variacoes) && p.variacoes.length) {
      const mapped = p.variacoes.map((raw) => {
        const v = raw as Record<string, unknown>;
        return {
          nome: nomeFromVariacao(v),
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editingId) {
        await updateMutation.mutateAsync({ id: editingId, ...payload } as never);
  ...
  async function handleDelete(id: number) {
    ...
    try {
      await deleteMutation.mutateAsync({ id } as never);
            ...
            <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-6">
              ...
              <div className="md:col-span-3">
                ...
                >
                  ...
                  <SelectContent>
                    {(Array.isArray(grupos) ? grupos : []).map((g: GrupoListItem) => (
                  ...
                  <div key={cs.corId} className="flex items-end gap-2 rounded-xl border bg-white p-3">
                    <div className="flex-1">
                      <div className="text-[11px] font-bold uppercase text-slate-500">
                        {(Array.isArray(cores) ? cores : []).find((c: CorListItem) => c.id === cs.corId)?.nome ?? "Cor"}
                      </div>
                      <Input
                        type="number"
                        className="mt-1 h-9"
