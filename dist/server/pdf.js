 ...
 */
export async function gerarBoletoPDF(boletoId) {
    const db_conn = await db.getDb();
    if (!db_conn)
        throw new ValidationError("Database not available");
 ...
 */
export async function gerarExtratoClientePDF(clienteId, vendedorId) {
    const db_conn = await db.getDb();
    if (!db_conn)
        throw new ValidationError("Database not available");
 ...
 */
export async function gerarBoletoPDFBytes(boletoId) {
    const db_conn = await db.getDb();
    if (!db_conn)
        throw new ValidationError("Database not available");
    const boleto = await db_conn.select().from(db.contasReceber).where(eq(db.contasReceber.id, boletoId)).limit(1);
...
export async function gerarRomaneioPDF(tenantId, cargaId) {
    ...
    const doc = new jsPDF();
    const numero = String(carga.numero || '').padStart(4, '0');
    const cidade = String(carga.cidadeRota || '').toUpperCase();
    const dataEntrega = carga.dataEntrega ? new Date(carga.dataEntrega) : new Date();
    ...
    function cleanText(s, max = 60) {
        const t = String(s ?? '').replace(/\s+/g, ' ').trim();
        if (!t)
            return '';
        return t.length > max ? t.slice(0, max - 1) + '…' : t;
    }
    function pagamentoResumo(p) {
        ...
        try {
            ...
            if (obj?.formaCombinada)
                return String(obj.formaCombinada);
        }
        catch { }
        return raw;
    }
    drawTableHeader();
    const itens = Array.isArray(carga.pedidos) ? carga.pedidos : [];
    ...
    for (const p of itens) {
        ...
        x += col.pedido;
        doc.text(cleanText(p.clienteNome, 30), x, y + 5.8);
        x += col.cliente;
        doc.text(cleanText(p.clienteTelefone, 14), x, y + 5.8);
        x += col.tel;
        doc.text(cleanText(p.vendedorNome, 18), x, y + 5.8);
        x += col.vend;
        doc.text(cleanText(pagamentoResumo(p.formaPagamento), 18), x, y + 5.8);
        x += col.pag;
        doc.text(money(p.total), x, y + 5.8);
        x += col.valor;
...
export async function gerarZipBoletos(params) {
 ...
 */
export async function gerarPedidoPDF(pedidoId) {
    const db_conn = await db.getDb();
    if (!db_conn)
        throw new ValidationError('Database not available');
...
export async function gerarBoletosCargaPDF(tenantId, cargaId, pedidoNumero) {
    ...
    const isCargaComPedidos = (x) => typeof x === "object" &&
        x !== null &&
        "pedidos" in x &&
        Array.isArray(x.pedidos) &&
        (x.pedidos.length === 0 ||
            typeof x.pedidos[0]?.numero === "number");
    ...
    const boletos = await db_conn.select().from(db.contasReceber)
        .where(and(inArray(db.contasReceber.pedidoNumero, pedidosFiltro), eq(db.contasReceber.formaPagamento, 'BOLETO'), eq(db.contasReceber.status, 'PENDENTE')));
    if (boletos.length === 0)
 ...
 */
export async function gerarRelatorioFinanceiroPDF(tipo, mesAno) {
    const db_conn = await db.getDb();
    if (!db_conn)
        throw new ValidationError("Database not available");
    const doc = new jsPDF();
    ...
    for (const item of dados) {
        ...
        const desc = (tipo === 'RECEBER' ? item.clienteNome : item.fornecedor) || 'S/D';
