const validationRules = {
    ...
    order: {
        ...
        validators: {
            clienteNome: (value) => {
                if (typeof value !== 'string')
                    return 'Client name must be a string';
                return value.trim().length >= 2 || 'Client name must be at least 2 characters';
            },
        ...
        validators: {
            valorConciliado: (value) => {
                if (value !== undefined && typeof value !== 'number')
                    return 'Reconciled value must be a number';
 ...
 */
export function validatePayload(entityType) {
    return (req, res, next) => {
        try {
 ...
 */
export function validateGenericPayload(req, res, next) {
    try {
        const payload = req.body;
