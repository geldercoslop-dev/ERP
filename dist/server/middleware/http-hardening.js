 ...
 */
function trustProxyMiddleware(req, res, next) {
 ...
 */
export { GLOBAL_RATE_LIMIT, SECURITY_HEADERS, trustProxyMiddleware };
/**
...
export function extractRealIP(req) {
    ...
    if (typeof fromXff === 'string' && fromXff.length > 0)
        return fromXff;
    if (Array.isArray(fromXff) && fromXff.length > 0)
        return fromXff[0] ?? 'unknown';
