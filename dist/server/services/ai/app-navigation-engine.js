 ...
 */
export async function processarComandoNavegacao(comando) {
 ...
 */
export function listarComandosDisponiveis() {
    return [
 ...
 */
export function sugerirComandos(parcial) {
    const parcialNormalizado = normalizarComando(parcial);
    const comandos = Object.keys(COMANDOS_NAVEGACAO);
    return comandos
        .filter((cmd) => cmd.includes(parcialNormalizado) || parcialNormalizado.includes(cmd))
        .slice(0, 5); // Limitar a 5 sugestões
}
/**
 * Verifica se uma rota é válida e existe no sistema.
 */
export async function validarRota(rota) {
    try {
        // Função getScreenByRoute removida - implementação segura
        const screen = null;
        return screen !== null;
