...
// Stubs para LEO ISOLADO
const leoLongMemory = {
    saveEvent: async (_tenantId, _event, _context, _importance) => { },
    saveInsight: async (_tenantId, _insight, _context, _importance) => { },
    getMemoryStats: async (_tenantId) => ({ count: 0, size: 0 }),
};
// Stub que retorna estrutura compatível
async function buildLeoContext(_source, _tenantId) {
    // LEO ISOLADO — retornando null para usar fallback
    return null;
}
import { aggregateTicketPedidos, sumPedidosTotalBetween, } from "./orders.service.js";
...
export async function askLeoQuestion(tenantId, question, context) {
    const startedAt = Date.now();
    await leoLongMemory.saveEvent(tenantId, `Usuário perguntou: ${question}`, context || "Pergunta via API", "medium");
    const answer = await processLeoQuestion(tenantId, question);
    await leoLongMemory.saveInsight(tenantId, `LEO respondeu: ${answer}`, `Resposta à pergunta: ${question}`, "medium");
    return {
        answer,
        processingTime: Date.now() - startedAt,
    };
}
export async function getLeoStatusSummary(tenantId, detailed) {
    const startedAt = Date.now();
    const basicStatus = {
        online: true,
    ...
    const [memoryStats, recentOrders, activeClientsPage, lowStockProducts, context,] = await Promise.all([
    ...
    const lowStockCount = lowStockProducts.filter((p) => Number(p.estoque ?? 0) < 10).length;
...
async function processLeoQuestion(tenantId, question) {
...
async function handleSalesQuestion(tenantId, question) {
    try {
        ...
        if (question.includes("ontem")) {
            ...
            const yesterdayTotal = await sumPedidosTotalBetween(tenantId, yesterdayStart, todayStart);
...
async function handleClientQuestion(tenantId, question) {
    try {
...
async function handleStockQuestion(tenantId) {
    try {
...
async function handleSystemStatusQuestion(tenantId) {
    try {
