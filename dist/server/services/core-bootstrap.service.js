...
import { ensureAdminUser, findOrCreateUserByOpenId, getVendedorByUserId, createVendedor, updateVendedorSenha, upsertUser, } from '../db/index.js';
 ...
 */
export async function ensureBootstrapAdminUser(tenantId = 1) {
    for (let i = 1; i <= 3; i++) {
        try {
 ...
 */
export async function ensureBootstrapK6User(tenantId = 1) {
 ...
 */
export async function checkBootstrapInitialization() {
    try {
