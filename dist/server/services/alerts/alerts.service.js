 ...
 */
export async function gerarAlertasOperacionais(tenantId, vendedorId) {
    return await runWithServiceInvocationAsync(buildBootstrapInvocation(1), async () => {
        assertTenantId(tenantId);
        const conn = await db.getDb();
        assertDbConnection(conn);
        const alertas = [];
        const pedidoWhereVendedor = vendedorId != null ? db.eq(db.pedidos.vendedorId, vendedorId) : undefined;
        ...
        const pedidosParados = await conn
            ...
            .from(db.pedidos)
            .where(pedidoWhereVendedor
            ? db.and(db.eq(db.pedidos.tenantId, tenantId), inArray(db.pedidos.status, [PedidoStatus.GERADO, PedidoStatus.PENDENTE_ESTOQUE]), db.sql `${db.pedidos.createdAt} < DATE_SUB(NOW(), INTERVAL 24 HOUR)`, pedidoWhereVendedor)
            : db.and(db.eq(db.pedidos.tenantId, tenantId), inArray(db.pedidos.status, [PedidoStatus.GERADO, PedidoStatus.PENDENTE_ESTOQUE]), db.sql `${db.pedidos.createdAt} < DATE_SUB(NOW(), INTERVAL 24 HOUR)`));
        ...
        const cargasAtrasadas = await conn
            ...
            .from(db.cargas)
            .where(db.and(db.eq(db.cargas.tenantId, tenantId), db.eq(db.cargas.status, CargaStatus.ABERTA), db.sql `${db.cargas.createdAt} < DATE_SUB(NOW(), INTERVAL 12 HOUR)`));
        ...
        const emRotaPendentes = await conn
            ...
            .from(db.pedidos)
            .where(pedidoWhereVendedor
            ? db.and(db.eq(db.pedidos.tenantId, tenantId), db.eq(db.pedidos.status, PedidoStatus.EM_ROTA), db.sql `${db.pedidos.updatedAt} < DATE_SUB(NOW(), INTERVAL 48 HOUR)`, pedidoWhereVendedor)
            : db.and(db.eq(db.pedidos.tenantId, tenantId), db.eq(db.pedidos.status, PedidoStatus.EM_ROTA), db.sql `${db.pedidos.updatedAt} < DATE_SUB(NOW(), INTERVAL 48 HOUR)`));
        ...
        const pedidosEntregues = await conn
            ...
            .where(pedidoWhereVendedor
            ? db.and(db.eq(db.pedidos.tenantId, tenantId), db.eq(db.contasReceber.tenantId, tenantId), db.eq(db.pedidos.status, PedidoStatus.ENTREGUE), db.eq(db.contasReceber.status, ContaReceberStatus.PENDENTE), pedidoWhereVendedor)
            : db.and(db.eq(db.pedidos.tenantId, tenantId), db.eq(db.contasReceber.tenantId, tenantId), db.eq(db.pedidos.status, PedidoStatus.ENTREGUE), db.eq(db.contasReceber.status, ContaReceberStatus.PENDENTE)));
        const seenFinanceiro = new Set();
        for (const p of pedidosEntregues) {
            if (seenFinanceiro.has(p.id))
                continue;
        ...
        const produtosBaixo = await conn
            ...
            .from(db.produtos)
            .where(db.and(db.eq(db.produtos.tenantId, tenantId), db.eq(db.produtos.ativo, 1), db.sql `${db.produtos.estoque} <= ${ESTOQUE_MINIMO_ALERTA}`));
