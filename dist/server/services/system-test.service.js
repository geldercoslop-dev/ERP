 ...
 */
export async function testConcurrency(produtoId = 1, quantidade = 5, testId = Date.now()) {
    const startTime = Date.now();
    try {
        console.log('[ConcurrencyTest] Iniciando teste de concorrência');
        // 1. Verificar estoque inicial (pool raw para execute)
        const pool = await getPool();
        const [initialStockRows] = await pool.execute(`SELECT id, descricao, estoque FROM produtos WHERE id = ?`, [produtoId]);
        ...
        const initialData = initialStock[0];
        ...
        const operacoes = [
            ...
            runStockTransaction(async (tx) => {
                ...
                const [produto] = await tx.execute(`SELECT id, descricao, estoque FROM produtos WHERE id = ? FOR UPDATE`, [produtoId]);
                ...
                await tx.execute(`UPDATE produtos SET estoque = ?, updatedAt = NOW() WHERE id = ?`, [novoEstoque, produtoId]);
                await tx.execute(`INSERT INTO audit_log (
            actorUserId, action, entity, entityId, payloadJson, traceId, createdAt
          ) VALUES (?, ?, ?, ?, ?, ?, NOW())`, [1, 'STOCK_DECREMENT', 'produtos', produtoId, JSON.stringify({
                        quantidade,
            ...
            runStockTransaction(async (tx) => {
                ...
                await tx.execute(`INSERT INTO audit_log (
            actorUserId, action, entity, entityId, payloadJson, traceId, createdAt
          ) VALUES (?, ?, ?, ?, ?, ?, NOW())`, [2, 'STOCK_DECREMENT', 'produtos', produtoId, JSON.stringify({
                        quantidade,
                        estoqueAnterior: estoqueAtual,
                        estoqueNovo: novoEstoque,
                        testId,
        ...
        const [finalStockRows] = await pool.execute(`SELECT id, descricao, estoque FROM produtos WHERE id = ?`, [produtoId]);
        ...
        if (process.env.NODE_ENV !== 'production' && !consistent) {
            await pool.execute(`UPDATE produtos SET estoque = ?, updatedAt = NOW() WHERE id = ?`, [estoqueInicial, produtoId]);
 ...
 */
export async function testDeadlock(produto1Id, produto2Id, testId = Date.now()) {
    const startTime = Date.now();
    try {
        ...
        const operacao1 = runTransaction(async (tx) => {
            ...
            await tx.execute(`SELECT * FROM produtos WHERE id = ? FOR UPDATE`, [produto1Id]);
            // Pequeno delay para aumentar chance de deadlock
            await new Promise(resolve => setTimeout(resolve, 50));
            console.log(`[DeadlockTest] Op1 - Tentando bloquear produto ${produto2Id}`);
            await tx.execute(`SELECT * FROM produtos WHERE id = ? FOR UPDATE`, [produto2Id]);
        ...
        const deadlockDetected = resultados.some(r => r.status === 'rejected' &&
