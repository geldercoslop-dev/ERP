...
import { getDb, getInsertId, clientes, clienteVendedores, vendedores, pedidos, insertAuditLog, normalizeTelefone, normalizeNomeSobrenome, } from "../db/core.js";
...
async function vendedorLinkedToCliente(dbConn, tenantId, vendedorId, clienteId) {
    const row = await dbConn
        .select({ id: clienteVendedores.id })
        .from(clienteVendedores)
        .innerJoin(clientes, eq(clientes.id, clienteVendedores.clienteId))
        .where(and(eq(clientes.tenantId, tenantId), eq(clienteVendedores.clienteId, clienteId), eq(clienteVendedores.vendedorId, vendedorId)))
        .limit(1);
    return row.length > 0;
}
/** Update/delete: apenas admin ou vendedor vinculado. */
function userCanMutateCliente(actor) {
    if (actor.role === "admin")
        return true;
...
async function userCanAccessCliente(dbConn, tenantId, actor, clienteId) {
    if (actor.role === "admin")
        return true;
    if (!actor.userId && !actor.vendedorId)
        return false;
 ...
 */
export async function createCliente(tenantId, data, userIdOverride) {
    try {
        ...
        if (!data.nome)
            return { success: false, error: "Nome do cliente obrigatório" };
        if (!data.telefone)
            return { success: false, error: "Telefone do cliente obrigatório" };
...
export async function getHistoricoCliente(tenantId, actor, clienteId, limit = 20) {
    try {
        if (!Number.isInteger(tenantId) || tenantId <= 0) {
            return { success: false, error: "tenantId obrigatório" };
        }
...
export async function getClienteById(tenantId, actor, id) {
    try {
        if (!Number.isInteger(tenantId) || tenantId <= 0) {
            return { success: false, error: "tenantId obrigatório" };
        }
...
export async function listClientes(tenantId, actor, params) {
    try {
        ...
        let effectiveVendedorFilter;
        let effectiveUserIdFilter;
        if (actor.role === "vendedor") {
        ...
        if (busca && busca.trim()) {
            ...
            conditions.push(sql `(${clientes.nome} LIKE ${searchTerm} OR 
        ...
        if (effectiveVendedorFilter != null) {
            ...
            const totalResult = await dbConn
                .select({ count: sql `count(distinct ${clientes.id})` })
                .from(clientes)
                .leftJoin(clienteVendedores, eq(clientes.id, clienteVendedores.clienteId))
                .where(and(...conditions, sql `${clienteVendedores.vendedorId} = ${effectiveVendedorFilter}`));
        ...
        const items = ensureArray(await dbConn.select().from(clientes).where(and(...conditions)).orderBy(asc(clientes.nome)).limit(limit).offset(offset));
        const totalResult = await dbConn.select({ count: sql `count(*)` }).from(clientes).where(and(...conditions));
        const total = Number(totalResult[0]?.count || 0);
        return { success: true, data: { items, total, page, pageSize } };
    }
    catch (error) {
...
export async function updateCliente(tenantId, actor, id, data) {
    try {
        ...
        const updateData = { updatedAt: toDbDateStrict(new Date()) };
        ...
        const fields = ['telefoneRecado', 'rua', 'numero', 'bairro', 'cidade', 'uf', 'referencia', 'condominio', 'bloco', 'apartamento'];
...
export async function associarClienteVendedor(tenantId, clienteId, vendedorId, principal = false) {
    try {
        if (!Number.isInteger(tenantId) || tenantId <= 0) {
            return { success: false, error: "tenantId obrigatório" };
        }
        ...
        if (existing.length > 0) {
            await dbConn.update(clienteVendedores)
                .set({ tipo: principal ? "PRINCIPAL" : "SECUNDARIO", createdAt: toDbDate(new Date()) })
...
export async function getOrCreateCliente(tenantId, data, tx) {
    try {
        if (!Number.isInteger(tenantId) || tenantId <= 0) {
            return { success: false, error: "tenantId obrigatório" };
        }
        ...
        const existing = await dbConn.select({ id: clientes.id })
            .from(clientes)
            .where(and(eq(clientes.tenantId, tenantId), eq(clientes.telefoneNorm, telefoneNorm), eq(clientes.nomeNorm, nn), eq(clientes.sobrenomeNorm, sn)))
            .limit(1);
        if (existing.length > 0) {
...
export async function getVendedorPrincipalDoCliente(tenantId, clienteId) {
    try {
        ...
        const result = await dbConn.select({
            ...
            .where(and(eq(clienteVendedores.clienteId, clienteId), eq(clienteVendedores.tipo, "PRINCIPAL"), eq(vendedores.tenantId, tenantId)))
            .limit(1);
...
export async function getVendedoresByCliente(tenantId, clienteId) {
    try {
        if (!Number.isInteger(tenantId) || tenantId <= 0) {
            return { success: false, error: "tenantId obrigatório" };
        }
        ...
        return { success: true, data: ensureArray(result) };
    }
    catch (error) {
        return { success: false, error: error instanceof Error ? error.message : String(error) };
    }
}
export async function getClienteByTelefone(tenantId, actor, telefone) {
    try {
        if (!telefone?.trim())
...
export async function searchClientesByNome(tenantId, actor, nome, limite = 10) {
    try {
        if (!nome?.trim())
        ...
        if (actor.role === "admin") {
            return { success: true, data: ensureArray(await dbConn
                    .select()
                    .from(clientes)
        ...
        const rows = await dbConn
            ...
            .orderBy(asc(clientes.nome))
            .limit(limite);
        return { success: true, data: rows.map((r) => r.c) };
    }
    catch (error) {
        return { success: false, error: error instanceof Error ? error.message : String(error) };
    }
}
export async function getReportClientesAtivos(tenantId, params) {
    try {
        if (!Number.isInteger(tenantId) || tenantId <= 0) {
            return { success: false, error: "tenantId obrigatório" };
        }
        ...
        try {
            const result = await db
                ...
                quantidadePedidos: sql `COUNT(*)`.as('quantidadePedidos')
            })
                .from(pedidos)
...
export async function listClientesComMetricasPedidos(tenantId, actor, limitRows = 500) {
    try {
        if (!Number.isInteger(tenantId) || tenantId <= 0) {
            return { success: false, error: "tenantId obrigatório" };
        }
        ...
        if (actor.role === "vendedor") {
            ...
            const rows = await dbConn
                ...
                nome: clientes.nome,
                createdAt: clientes.createdAt,
                totalPedidos: sql `(SELECT COUNT(*) FROM pedidos WHERE pedidos.cliente_id = ${clientes.id} AND pedidos.vendedor_id = ${actor.vendedorId})`.as("totalPedidos"),
                totalGasto: sql `(SELECT COALESCE(SUM(pedidos.total), 0) FROM pedidos WHERE pedidos.cliente_id = ${clientes.id} AND pedidos.vendedor_id = ${actor.vendedorId})`.as("totalGasto"),
                avgTicket: sql `(SELECT COALESCE(AVG(pedidos.total), 0) FROM pedidos WHERE pedidos.cliente_id = ${clientes.id} AND pedidos.vendedor_id = ${actor.vendedorId})`.as("avgTicket"),
            })
                .from(clientes)
                .innerJoin(clienteVendedores, eq(clientes.id, clienteVendedores.clienteId))
        ...
        const rows = await dbConn
            ...
            nome: clientes.nome,
            createdAt: clientes.createdAt,
            totalPedidos: sql `(SELECT COUNT(*) FROM pedidos WHERE pedidos.cliente_id = ${clientes.id})`.as("totalPedidos"),
            totalGasto: sql `(SELECT COALESCE(SUM(pedidos.total), 0) FROM pedidos WHERE pedidos.cliente_id = ${clientes.id})`.as("totalGasto"),
            avgTicket: sql `(SELECT COALESCE(AVG(pedidos.total), 0) FROM pedidos WHERE pedidos.cliente_id = ${clientes.id})`.as("avgTicket"),
        })
            .from(clientes)
