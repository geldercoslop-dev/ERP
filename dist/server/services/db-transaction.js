 ...
 */
export async function runTransaction(callback, isolationLevel = 'READ_COMMITTED') {
 ...
 */
export async function runParallelTransaction(callbacks, isolationLevel = 'SERIALIZABLE') {
 ...
 */
export async function isTableLocked(tableName) {
    // Obter conexão do pool em vez de criar uma nova
    const pool = await getConnectionPool();
    const connection = await pool.getConnection();
    try {
        const [rows] = await connection.execute(`SELECT COUNT(*) as locked FROM INFORMATION_SCHEMA.INNODB_LOCKS WHERE TABLE_NAME = ?`, [tableName]);
        const row0 = rows[0];
 ...
 */
export async function runStockTransaction(callback, maxRetries = 3, baseDelay = 1000) {
    let lastError;
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            const isRetryableError = errorMessage.includes('Deadlock found') ||
            ...
            if (isRetryableError && attempt < maxRetries) {
                ...
                const delay = Math.min(baseDelay * Math.pow(2, attempt - 1) + Math.random() * 1000, 30000 // Máximo de 30 segundos
                );
 ...
 */
export async function runTransactionWithRetry(callback, options = {}) {
    const { isolationLevel = 'READ_COMMITTED', maxRetries = 3, baseDelay = 1000, retryableErrors = ['Deadlock found', 'Lock wait timeout', 'Duplicate entry', 'Connection lost', 'Server has gone away'] } = options;
    let lastError;
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            lastError = error;
            const errorMessage = error instanceof Error ? error.message : String(error);
            const isRetryableError = retryableErrors.some(retryableError => errorMessage.includes(retryableError));
            if (isRetryableError && attempt < maxRetries) {
                // Exponential backoff com jitter
                const delay = Math.min(baseDelay * Math.pow(2, attempt - 1) + Math.random() * 1000, 30000 // Máximo de 30 segundos
                );
