 ...
 */
async function getMigrationsAppliedCount(db) {
    try {
        const res = await db.execute(sql `SELECT COUNT(*) as count FROM __drizzle_migrations`);
        const rows = res.rows;
    ...
    bootstrapOnce = (async () => {
        ...
        try {
            ...
            try {
                ...
                const ms = Date.now() - startedAt;
                console.error(`[BOOTSTRAP][DB] ❌ migration falhou após ${ms}ms - FAIL-HARD`);
                console.error(`[BOOTSTRAP][DB] Erro: ${errorMsg}`);
                // FAIL-HARD: migration falhou → derrubar sistema
                throw new InfrastructureError(`Migration falhou: ${errorMsg}`);
            }
            return { success: true };
        }
        catch (err) {
            ...
            const ms = Date.now() - startedAt;
            console.error(`[BOOTSTRAP][DB] ❌ erro de contexto após ${ms}ms - FAIL-HARD`);
            console.error(`[BOOTSTRAP][DB] Erro: ${errorMsg}`);
            // FAIL-HARD: erro de contexto → derrubar sistema
            throw new InfrastructureError(`Bootstrap context error: ${errorMsg}`);
        }
    })();
    return bootstrapOnce;
}
let bootstrapOnce = null;
