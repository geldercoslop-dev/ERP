...
export class SalesAnalyticsQueries {
     ...
     */
    static async getVendasUltimos30Dias(limit = 30) {
     ...
     */
    static async getProdutosMaisVendidos(limit = 10) {
        const dbConnection = await getDb();
        assertDbConnection(dbConnection);
        try {
            const trintaDiasAtras = new Date();
     ...
     */
    static async getClientesMaisAtivos(limit = 10) {
        const dbConnection = await getDb();
        assertDbConnection(dbConnection);
        try {
            const trintaDiasAtras = new Date();
...
export class StockAnalyticsQueries {
     ...
     */
    static async getEstoqueCritico(limit = 50) {
        const dbConnection = await getDb();
        assertDbConnection(dbConnection);
        try {
            const query = `
     ...
     */
    static async getProdutosSemGiro(dias = 60, limit = 50) {
        const dbConnection = await getDb();
        assertDbConnection(dbConnection);
        try {
     ...
     */
    static async getProdutosAltoGiro(dias = 30, limit = 20) {
        const dbConnection = await getDb();
        assertDbConnection(dbConnection);
        try {
            const dataLimite = new Date();
...
export class FinancialInsightsQueries {
     ...
     */
    static async getFaturamentoDiario(dias = 30, limit = 100) {
        const dbConnection = await getDb();
        assertDbConnection(dbConnection);
        try {
            const dataLimite = new Date();
        ...
        LIMIT ?
      `;
            const [rows] = await executeQuery(query, [dataLimite, limit]);
            return rows;
        }
        catch (error) {
     ...
     */
    static async getFluxoCaixa(dias = 30, limit = 50) {
        const dbConnection = await getDb();
        assertDbConnection(dbConnection);
        try {
            const dataLimite = new Date();
     ...
     */
    static async getIndicadoresFinanceiros() {
        const dbConnection = await getDb();
        if (!dbConnection)
            return null;
        try {
      ...
      `;
            ...
            return {
                ...
                healthScore: Math.min(100, Math.max(0, (contasRec > 0 ? 30 : 0) +
                    (crescimento > 0 ? 30 : 0) +
                    (ticket > 100 ? 20 : 0) +
                    (totalPed > 10 ? 20 : 0))),
            };
