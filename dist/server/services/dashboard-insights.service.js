...
export async function getDashboardInsights(tenantId) {
    ...
    if (!conn)
        return empty;
    const [produtosQueVaoFaltar, clientesInativos, vendedoresAbaixoMedia, pedidosAtrasados, contasVencidas] = await Promise.all([
...
async function getProdutosQueVaoFaltar(conn, tenantId) {
    ...
    for (const p of ruptura) {
        ...
        if (estoqueProjetado >= minStock && p.diasAteRuptura >= 30)
            continue;
...
async function getClientesInativos(conn, tenantId) {
    const limite = new Date();
    limite.setDate(limite.getDate() - DIAS_INATIVIDADE_CLIENTE);
    const sub = conn
        .select({
        clienteId: db.pedidos.clienteId,
        ultimaCompra: sql `MAX(${db.pedidos.createdAt})`.as("ultimaCompra"),
        valorTotalHistorico: sql `COALESCE(SUM(${db.pedidos.total}), 0)`.as("valorTotalHistorico"),
    })
        .from(db.pedidos)
...
async function getVendedoresAbaixoMedia(conn, tenantId) {
    const inicio = new Date();
    inicio.setDate(inicio.getDate() - DIAS_VENDAS_VENDEDOR);
    const vendasPorVendedor = await conn
        .select({
        vendedorId: db.pedidos.vendedorId,
        totalVendido: sql `COALESCE(SUM(${db.pedidos.total}), 0)`.as("totalVendido"),
    })
        .from(db.pedidos)
        .where(and(eq(db.pedidos.tenantId, tenantId), ne(db.pedidos.status, PedidoStatusValues[4]), sql `${db.pedidos.createdAt} >= ${inicio}`))
    ...
    if (vendedoresIds.length === 0)
        return []; // Ausência legítima - sem vendedores no período
    ...
    const mapaNome = new Map(vendedores.map((v) => [v.id, String(v.nome ?? "")]));
    const resultado = [];
    for (const r of vendasPorVendedor) {
        const total = Number(r.totalVendido ?? 0);
        if (media <= 0 || total >= limiteAbaixo)
            continue;
...
async function getPedidosAtrasados(conn, tenantId) {
    const hoje = new Date();
    ...
    const rows = await conn
        ...
        .from(db.pedidos)
        .where(and(eq(db.pedidos.tenantId, tenantId), ne(db.pedidos.status, PedidoStatusValues[3]), sql `${db.pedidos.dataEntrega} IS NOT NULL`, lt(db.pedidos.dataEntrega, toDbDateStrict(hoje))));
    return rows.map((r) => {
...
async function getContasVencidas(conn, tenantId) {
    const hoje = new Date();
    ...
    const rows = await conn
        ...
        .where(and(eq(db.contasReceber.tenantId, tenantId), eq(db.contasReceber.status, ContaReceberStatusValues[0]), lt(db.contasReceber.dataVencimento, toDbDateStrict(hoje))))
