 ...
 */
import { validateTenantOwnershipByUserId, } from './tenant-validation.service.js';
/**
 * Valida se o userId realmente pertence ao tenantId informado
 * Busca no banco para garantir que não é spoofing
 */
export async function validateCoreTenantOwnership(userId, tenantId) {
    // Validações básicas
    if (!userId || !tenantId) {
        return {
            valid: false,
 ...
 */
export async function validateCoreSecureContext(userId, tenantId, userRole) {
