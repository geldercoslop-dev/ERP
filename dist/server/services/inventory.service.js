...
import { getDb, getInsertId, insertAuditLog } from "../db/index.js";
import { produtos, cores, pendencias, gruposPrecificacao, } from "../../drizzle/schema.js";
 ...
 * CORES
 */
export async function listCores(tenantId) {
...
export async function createCor(tenantId, data) {
    assertTenantId(tenantId);
    const dbConn = await getDb();
    assertDbConnection(dbConn);
...
export async function updateCor(tenantId, id, data) {
    assertTenantId(tenantId);
    const dbConn = await getDb();
    assertDbConnection(dbConn);
    ...
    if (!after.length)
        throw new InfrastructureError("Falha ao atualizar cor");
    return { success: true };
}
export async function deleteCor(tenantId, id) {
    ...
    const after = await dbConn.select().from(cores).where(and(eq(cores.tenantId, tenantId), eq(cores.id, id))).limit(1);
    if (after.length > 0)
        throw new InfrastructureError("Falha ao excluir cor");
    return { success: true };
}
/**
 * PRODUTOS
...
export async function getAllProdutos(tenantId) {
    assertTenantId(tenantId);
    const dbConn = await getDb();
    assertDbConnection(dbConn);
...
export async function getAllProdutosComPrecoVigente(tenantId, refDate = new Date()) {
  ...
  `;
    ...
    const result = safeRows.map((p) => {
        ...
        if (cor)
            descricaoOperacional += ` - ${cor}`;
        if (tamanho)
            descricaoOperacional += ` (${tamanho})`;
        if (temEspelho)
            descricaoOperacional += " COM ESPELHO";
...
export async function getProdutosComPrecoVigentePaged(tenantId, opts) {
  ...
  `;
    ...
    const countRows = ensureArray(countResult[0]);
...
export async function updateEstoqueProduto(tenantId, input) {
    const dbConn = await getDb();
    assertDbConnection(dbConn);
    ...
    const current = res[0];
    if (!current)
        throw new ValidationError("Produto não encontrado ou acesso negado");
    ...
    return { ...out, id: input.id, estoque: saldoAtual + delta };
}
export async function getProdutosEstoqueBaixo(tenantId, limite) {
    assertTenantId(tenantId);
    const dbConn = await getDb();
    assertDbConnection(dbConn);
    const rows = await dbConn
        .select()
        .from(produtos)
        .where(and(eq(produtos.tenantId, tenantId), eq(produtos.ativo, 1)))
        .orderBy(asc(produtos.estoque))
        .limit(Math.max(1, Math.min(limite, 500)));
    // Garantir que rows seja um array
    const safeRows = ensureArray(rows);
    const items = safeRows.map((r) => ({ ...r }));
...
export async function updateProduto(tenantId, id, data, version) {
    try {
        ...
        if (version !== undefined) {
            ...
            if (updatedAt && new Date(updatedAt) > new Date(version))
                throw new ValidationError(`O produto foi modificado por outro usuário desde que você o abriu.`);
        }
        await dbConn.update(produtos).set({ ...data, updatedAt: toDbDate(new Date()) }).where(and(eq(produtos.tenantId, tenantId), eq(produtos.id, id)));
        const after = await getProdutoById(tenantId, id);
        if (!after)
            throw new InfrastructureError("Falha ao atualizar produto");
        return { success: true };
    }
    catch (error) {
...
export async function deleteProduto(tenantId, id) {
    try {
        ...
        const dbConn = await getDb();
        assertDbConnection(dbConn);
        await dbConn.update(produtos).set({ ativo: 0, updatedAt: toDbDate(new Date()) }).where(and(eq(produtos.tenantId, tenantId), eq(produtos.id, id)));
        const after = await getProdutoById(tenantId, id);
        if (!after || after.ativo !== 0)
            throw new InfrastructureError("Falha ao excluir logicamente produto");
        return { success: true };
    }
    catch (error) {
...
export async function ajusteRapidoEstoque(tenantId, produtoId, quantidade, tipo, audit) {
    ...
    if (tipo === "entrada" && ajuste > 0) {
        ...
        const pendenciasAtivas = await dbConn.select().from(pendencias)
            .where(and(eq(pendencias.tenantId, tenantId), eq(pendencias.produtoId, produtoId), inArray(pendencias.status, [PendenciaStatus.PENDENTE, PendenciaStatus.COMPRADO])))
        ...
        for (const p of pendenciasAtivas) {
            ...
            if (qtdParaBaixa === p.quantidade) {
                await dbConn
                    .update(pendencias)
                    .set({ status: PendenciaStatus.RESOLVIDO, dataResolvido: toDbDate(new Date()) })
...
export async function criarNotaEntrada(tenantId, input) {
    const dbConn = await getDb();
    assertDbConnection(dbConn);
    const txSqlRunner = (tx) => tx;
    await dbConn.transaction(async (tx) => {
        // 1. Inserir nota (exemplo simplificado, idealmente via schema)
        const [notaRes] = await txSqlRunner(tx).execute("INSERT INTO notas_entrada (tenantId, marca, dataChegada, valorTotal, formaPagamento, observacao, createdBy) VALUES (?, ?, ?, ?, ?, ?, ?)", [tenantId, input.marca, input.dataChegada, input.valorTotal, input.formaPagamento, input.observacao ?? null, input.createdBy ?? null]);
        const notaPacket = notaRes;
        const notaId = Number(notaPacket.insertId ?? 0);
        if (!notaId)
            throw new InfrastructureError("Falha ao criar nota de entrada");
        // 2. Financeiro
        ...
        for (const it of input.itens) {
            await txSqlRunner(tx).execute('INSERT INTO notas_entrada_itens (tenantId, notaId, produtoId, quantidade, custoUnit) VALUES (?, ?, ?, ?, ?)', [tenantId, notaId, it.produtoId, it.quantidade, it.custoUnit ?? null]);
            // Primeiro obter o estoque atual
...
export async function countProdutosAtivosEstoqueAte(tenantId, maxInclusive) {
    assertTenantId(tenantId);
    const dbConn = await getDb();
    assertDbConnection(dbConn);
    const [row] = await dbConn
        ...
        .from(produtos)
        .where(and(eq(produtos.tenantId, tenantId), eq(produtos.ativo, 1), lte(produtos.estoque, maxInclusive)))
        .limit(1);
    return Number(row?.c ?? 0);
}
export async function countProdutosAtivosEstoqueZero(tenantId) {
    return countProdutosAtivosEstoqueAte(tenantId, 0);
}
export async function getProdutosComEstoqueAte(tenantId, maxInclusive) {
    assertTenantId(tenantId);
    const dbConn = await getDb();
    assertDbConnection(dbConn);
    return dbConn
...
export async function listProdutosResumoLeoLearning(tenantId) {
    assertTenantId(tenantId);
    const dbConn = await getDb();
    assertDbConnection(dbConn);
    return dbConn
...
export async function listGruposPrecificacao(tenantId) {
    assertTenantId(tenantId);
    const dbConn = await getDb();
    assertDbConnection(dbConn);
    return await dbConn.select().from(gruposPrecificacao).where(eq(gruposPrecificacao.tenantId, tenantId)) ?? [];
}
export async function listProdutosBaixoEstoqueLeo(tenantId, limite) {
    assertTenantId(tenantId);
    const dbConn = await getDb();
    assertDbConnection(dbConn);
    return dbConn
        ...
        .from(produtos)
        .where(and(eq(produtos.tenantId, tenantId), eq(produtos.ativo, 1), lte(produtos.estoque, limite)))
        .orderBy(asc(produtos.estoque));
}
export async function listProdutoVendasStatsLeoLearning(tenantId) {
    assertTenantId(tenantId);
    const dbConn = await getDb();
    assertDbConnection(dbConn);
    const rows = await dbConn
        ...
        categoria: produtos.categoria,
        vendasCount: sql `(SELECT COUNT(*) FROM itens_pedido i INNER JOIN pedidos p ON p.id = i.pedido_id WHERE i.produto_id = ${produtos.id} AND p.tenant_id = ${tenantId})`.as("vendasCount"),
        receita: sql `(SELECT COALESCE(SUM(i.quantidade * i.valor_unitario), 0) FROM itens_pedido i INNER JOIN pedidos p ON p.id = i.pedido_id WHERE i.produto_id = ${produtos.id} AND p.tenant_id = ${tenantId})`.as("receita"),
    })
        .from(produtos)
        .where(eq(produtos.tenantId, tenantId));
    return rows.map((r) => ({
