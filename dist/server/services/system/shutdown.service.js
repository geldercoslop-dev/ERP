...
export function markShutdownBootComplete() {
...
const SHUTDOWN_FORCE_EXIT_MS = Math.max(1000, Number(process.env.SHUTDOWN_FORCE_EXIT_MS || 10_000));
...
export function attachShutdownHttpServer(server, options = {}) {
    httpServer = server;
...
async function closeHttpServer() {
    ...
    await Promise.race([
        ...
        new Promise((_, reject) => setTimeout(() => reject(new Error("HTTP close timeout")), 8000)),
    ]).catch((err) => {
...
async function closeDatabase() {
    ...
    try {
        await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error("DB close timeout")), 8000)),
        ]);
...
async function closeRedisConnection() {
    ...
    try {
        await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error("Redis close timeout")), 8000)),
        ]);
...
async function shutdownWithExit(signal, exitCode) {
 ...
 */
export async function requestShutdown(payload) {
    const exitCode = payload.code === 1 ? 1 : 0;
 ...
 */
export async function initiateGracefulShutdown(signal = "manual", exitCode = 0) {
...
export function registerProcessShutdownHandlers() {
    ...
    const blockSigintShutdown = process.env.BLOCK_SIGINT_SHUTDOWN === "1" || !process.stdin.isTTY;
