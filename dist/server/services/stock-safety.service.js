...
function mapProdutoRowsToLock(rows) {
    const m = new Map();
 ...
 */
export async function updateStockSafe(tenantId, // Adicionado para multi-tenant
produtoId, quantidade, operacao) {
    ...
    try {
        const out = await runStockTransaction(async (tx) => {
            ...
            const [produto] = await tx.execute(`SELECT id, descricao, estoque, ativo FROM produtos WHERE tenantId = ? AND id = ? FOR UPDATE`, [tenantId, produtoId]);
            ...
            const updateResult = await tx.execute(`UPDATE produtos SET estoque = ?, updatedAt = NOW() WHERE tenantId = ? AND id = ? AND estoque = ?`, [novoEstoque, tenantId, produtoId, estoqueAtual]);
            ...
            try {
                await tx.execute(`INSERT INTO stock_movements (produtoId, descricao, quantidade, tipo, estoqueAnterior, estoqueNovo, motivo, usuario, timestamp, tenantId) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [produtoId, produtoData.descricao, quantidade, operacao.tipo, estoqueAtual, novoEstoque, operacao.motivo, movement.usuario, movement.timestamp, tenantId]);
            }
            catch (auditError) {
 ...
 */
export async function validateStockAvailability(tenantId, // Adicionado para multi-tenant
produtoId, quantidade, tipo) {
    try {
        ...
        try {
            const [produto] = await dbConnection.execute(`SELECT id, descricao, estoque, ativo FROM produtos WHERE tenantId = ${tenantId} AND id = ${produtoId}`);
            if (!produto || !Array.isArray(produto) || produto.length === 0) {
 ...
 */
export async function updateMultipleStockSafe(tenantId, // Adicionado para multi-tenant
operacoes) {
    assertTenantId(tenantId);
    try {
        return await runStockTransaction(async (tx) => {
            ...
            const produtosQuery = await tx.execute(`SELECT id, descricao, estoque, ativo FROM produtos WHERE tenantId = ? AND id IN (${produtoIds.map(() => '?').join(',')}) FOR UPDATE`, [tenantId, ...produtoIds]);
            ...
            for (const operacao of operacoes) {
                ...
                // Atualizar estoque
                await tx.execute(`UPDATE produtos SET estoque = ?, updatedAt = NOW() WHERE tenantId = ? AND id = ? AND estoque = ?`, [novoEstoque, tenantId, operacao.produtoId, estoqueAtual]);
                // Registrar movimento
                const movement = {
                    id: 0,
 ...
 */
export async function reserveStockForOrder(tenantId, // Adicionado para multi-tenant
itens, pedidoId, usuarioId, vendedorId) {
    assertTenantId(tenantId);
    try {
        return await runStockTransaction(async (tx) => {
            const movements = [];
            ...
            for (const item of itens) {
                ...
                // Atualizar estoque
                await tx.execute(`UPDATE produtos SET estoque = ?, updatedAt = NOW() WHERE tenantId = ? AND id = ? AND estoque = ?`, [novoEstoque, tenantId, item.produtoId, estoqueAtual]);
                // Registrar movimento
                const movement = {
                    id: 0,
 ...
 */
export async function releaseStockForOrder(tenantId, // Adicionado para multi-tenant
itens, pedidoId, usuarioId, vendedorId) {
    assertTenantId(tenantId);
    try {
        return await runStockTransaction(async (tx) => {
 ...
 */
export async function getStockMovements(tenantId, filters = {}) {
    try {
        const dbConnection = await db.getDb();
        if (!dbConnection) {
            return { success: false, error: "Database connection not available" };
    ...
    `;
        ...
        const movements = Array.isArray(result) ? result.map((row) => ({
            id: row.id,
 ...
 */
export async function reconstituteStockFromMovements(tenantId, produtoId, dataReferencia) {
    try {
        const dbConnection = await db.getDb();
        if (!dbConnection) {
            return { success: false, error: "Database connection not available" };
    ...
    `;
        const params = [tenantId, produtoId];
        if (dataReferencia) {
            query += ` AND timestamp <= ?`;
 ...
 */
export async function validateStockIntegrity(tenantId, produtoId) {
    try {
        const dbConnection = await db.getDb();
        if (!dbConnection) {
            return { success: false, error: "Database connection not available" };
    ...
    `;
        const [result] = await dbConnection.execute(query);
        const discrepancies = Array.isArray(result) ? result.map((row) => ({
            produtoId: row.produtoId,
            descricao: row.descricao,
 ...
 */
export async function recoverStockMovementsFromAudit(tenantId, limit = 1000) {
    try {
        const dbConnection = await db.getDb();
        if (!dbConnection) {
            return { success: false, error: "Database connection not available" };
