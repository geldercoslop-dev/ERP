...
import { clienteVendedores } from "../../db/index.js";
...
function asCargaRecord(carga) {
    return isRecord(carga) ? carga : {};
}
function pedidosCargaLista(carga) {
    const p = carga["pedidos"];
    if (!Array.isArray(p))
        return []; // Ausência legítima - sem pedidos na carga
 ...
 */
export async function gerarBoletoPDF(tenantId, boletoId) {
 ...
 */
export async function gerarExtratoClientePDF(tenantId, clienteId, vendedorId) {
    const db_conn = await db.getDb();
    if (!db_conn)
    ...
    if (vendedorId != null)
        whereParts.push(eq(db.boletos.vendedorId, vendedorId));
 ...
 */
export async function gerarBoletoPDFBytes(tenantId, boletoId) {
    const db_conn = await db.getDb();
    if (!db_conn)
        return { success: false, error: "Database not available" };
    const result = await db_conn.select().from(db.contasReceber)
 ...
 */
export async function gerarRomaneioPDF(tenantId, cargaId) {
    ...
    const dataEntrega = de instanceof Date ? de : typeof de === "string" || typeof de === "number" ? new Date(de) : new Date();
    ...
    function drawTableHeader() {
        ...
        doc.setFontSize(9);
        let x = x0 + 2;
        doc.text('PED', x, y + 5.5);
        x += col.pedido;
        doc.text('CLIENTE', x, y + 5.5);
        x += col.cliente;
        doc.text('TEL', x, y + 5.5);
        x += col.tel;
        doc.text('VEND', x, y + 5.5);
        x += col.vend;
        doc.text('PAGTO', x, y + 5.5);
        x += col.pag;
        doc.text('R$', x, y + 5.5);
        x += col.valor;
    ...
    itens.sort((a, b) => String(a["clienteNome"] ?? "").localeCompare(String(b["clienteNome"] ?? ""), "pt-BR"));
    let zebra = false;
    for (const p of itens) {
        ...
        doc.setTextColor(0, 0, 0);
        let x = x0 + 2;
        doc.text(String(p["numero"] ?? ""), x, y + 5.8);
        x += col.pedido;
        doc.text(String(p["clienteNome"] ?? "").slice(0, 30), x, y + 5.8);
        x += col.cliente;
        doc.text(String(p["clienteTelefone"] ?? "").slice(0, 14), x, y + 5.8);
        x += col.tel;
        doc.text(String(p["vendedorNome"] ?? "").slice(0, 18), x, y + 5.8);
        x += col.vend;
        doc.text(String(p["formaPagamento"] ?? "").slice(0, 18), x, y + 5.8);
        x += col.pag;
        doc.text(Number(p["total"] ?? 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 }), x, y + 5.8);
        x += col.valor;
 ...
 */
export async function gerarRoteiroEntregaPDF(tenantId, cargaId) {
    const carga = await logisticaService.getCargaById(tenantId, cargaId);
    if (!carga)
        throw new ValidationError('Carga não encontrada');
    const cr = asCargaRecord(carga);
    ...
    const dataEntrega = deR instanceof Date ? deR : typeof deR === "string" || typeof deR === "number" ? new Date(deR) : new Date();
    const dataStr = dataEntrega.toLocaleDateString('pt-BR');
    ...
    function drawTableHeader() {
        ...
        x += col.valor;
        doc.text('BAIRRO', x, y + 5.5);
        x += col.bairro;
        doc.text('CIDADE', x, y + 5.5);
        x += col.cidade;
        doc.text('VEND', x, y + 5.5);
        x += col.vendedor;
        doc.text('HORÁRIO', x, y + 5.5);
        x += col.horario;
        doc.text('OBS', x, y + 5.5);
        y += headerH;
    }
    ...
    for (const p of itens) {
        ...
        doc.setFontSize(8);
        let x = x0 + 2;
        doc.text(String(Number(p["ordemEntrega"] ?? 0) + 1), x, y + 5.5);
        x += col.ordem;
        doc.text(String(p["numero"] ?? ""), x, y + 5.5);
        x += col.pedido;
        doc.text(String(p["clienteNome"] ?? "").slice(0, 22), x, y + 5.5);
        x += col.cliente;
        doc.text(Number(p["total"] ?? 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 }), x, y + 5.5);
        x += col.valor;
        doc.text(String(p["bairro"] ?? p["clienteBairro"] ?? "").slice(0, 16), x, y + 5.5);
        x += col.bairro;
        doc.text(String(p["cidade"] ?? p["clienteCidade"] ?? "").slice(0, 14), x, y + 5.5);
        x += col.cidade;
        doc.text(String(p["vendedorNome"] ?? "").slice(0, 12), x, y + 5.5);
        x += col.vendedor;
        doc.text(String(p["horarioPrevisto"] ?? "-"), x, y + 5.5);
        x += col.horario;
 ...
 */
export async function gerarZipBoletos(tenantId, params) {
    const { boletoIds, pedidoNumero, clienteNome } = params;
    if (!boletoIds.length)
        throw new ValidationError('Nenhum boleto para gerar ZIP');
 ...
 */
export async function gerarEstoquePDF(tenantId) {
 ...
 */
export async function gerarVendasPDF(tenantId, params) {
    const db_conn = await db.getDb();
    if (!db_conn)
        throw new InfrastructureError("Database not available");
    const vendas = await db_conn.select({
        data: sql `DATE(${db.pedidos.createdAt})`,
        quantidade: sql `COUNT(*)`,
        valor: sql `SUM(${db.pedidos.total})`
    })
        .from(db.pedidos)
        .where(and(eq(db.pedidos.tenantId, tenantId), sql `${db.pedidos.createdAt} >= ${params.dataInicio}`, sql `${db.pedidos.createdAt} <= ${params.dataFim}`))
    ...
    const margin = 20;
    const totalPedidos = vendas.reduce((acc, v) => acc + Number(v.quantidade), 0);
 ...
 */
export async function gerarPedidoPDF(tenantId, pedidoId) {
 ...
 */
export async function gerarBoletosCargaPDF(tenantId, cargaId, pedidoNumero) {
    const carga = await logisticaService.getCargaById(tenantId, cargaId);
    if (!carga)
        throw new ValidationError("Carga não encontrada");
    const db_conn = await db.getDb();
    ...
    const boletos = await db_conn.select().from(db.contasReceber)
        .where(and(eq(db.contasReceber.tenantId, tenantId), inArray(db.contasReceber.pedidoNumero, pedidosFiltro), eq(db.contasReceber.formaPagamento, 'BOLETO'), eq(db.contasReceber.status, ContaReceberStatus.PENDENTE)));
    if (boletos.length === 0)
        throw new ValidationError("Nenhum boleto encontrado.");
    const doc = new jsPDF();
    const dadosBanco = await db.getConfig("DADOS_BANCO") || "DADOS BANCÁRIOS NÃO CONFIGURADOS";
    const chavePix = await db.getConfig("CHAVE_PIX") || "PIX NÃO CONFIGURADO";
 ...
 */
export async function gerarRelatorioLeoPDF(tenantId, tipo, actor) {
        ...
        case 'clientes':
            ...
            if (actor?.role === 'vendedor' && actor?.userId) {
                ...
                const clienteVendedorIds = await db_conn.select({ clienteId: clienteVendedores.clienteId })
                    ...
                    .where(and(eq(clienteVendedores.vendedorId, actor.vendedorId || 0)));
                const clienteIds = clienteVendedorIds.map(cv => cv.clienteId);
                clientesList = await db_conn.select().from(clientes)
                    .where(and(eq(clientes.tenantId, tenantId), inArray(clientes.id, clienteIds)))
                    .limit(1000);
            }
            else {
 ...
 */
export async function gerarRelatorioFinanceiroPDF(tenantId, tipo, mesAno) {
    const db_conn = await db.getDb();
    if (!db_conn)
        throw new InfrastructureError("Database not available");
    const doc = new jsPDF();
