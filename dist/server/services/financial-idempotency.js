 ...
 */
export function generateIdempotencyKey(operationType, resourceId, additionalData) {
 ...
 */
export async function checkOperationProcessed(tenantId, operationKey, operationType) {
        ...
        if (result.length > 0) {
            ...
            return {
                ...
                record: {
                    ...
                    operationType: typeof record.operation_type === 'string' ? record.operation_type : 'BAIXA_BOLETO',
 ...
 */
export async function markOperationProcessed(tenantId, operationKey, operationType, metadata) {
    const db = await getDb();
    if (!db)
        throw new InfrastructureError("Database not available");
 ...
 */
export async function financialIdempotencyCheck(tenantId, operationType, resourceId, operationData) {
 ...
 */
export async function executeWithIdempotency(tenantId, operationType, resourceId, operation, operationData) {
    // Gerar chave
    const operationKey = generateIdempotencyKey(operationType, resourceId, operationData);
    // Verificar se já foi processado
    const check = await checkOperationProcessed(tenantId, operationKey, operationType);
 ...
 */
export async function cleanupOldIdempotencyRecords(daysToKeep = 90) {
    const db = await getDb();
    if (!db)
        throw new InfrastructureError("Database not available");
    const result = await db.execute(sql `
