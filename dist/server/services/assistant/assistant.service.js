...
function proximaDataComercial(dia, mes) {
    const hoje = new Date();
    let ano = hoje.getFullYear();
    let d = new Date(ano, mes - 1, dia);
    if (d < hoje)
        d = new Date(ano + 1, mes - 1, dia);
    return d;
}
/** Retorna true se hoje está no intervalo [data - 15 dias, data]. */
function estaProximo(data, diasAntes) {
    const hoje = new Date();
 ...
 */
export async function gerarSugestoesSistema(tenantId) {
    try {
        ...
        const produtosComEstoque = await conn
            ...
            .from(db.produtos)
            .where(db.and(db.eq(db.produtos.tenantId, tenantIdNum), db.sql `${db.produtos.estoque} > 0`, db.eq(db.produtos.ativo, 1)));
        const ultimaVendaPorProduto = await conn
            .select({
            produtoId: db.itensPedido.produtoId,
            ultimaData: db.sql `MAX(${db.pedidos.updatedAt})`.as("ultimaData"),
        })
            .from(db.itensPedido)
        ...
        for (const r of ultimaVendaPorProduto) {
            if (r.produtoId != null)
                ultimaPorId.set(r.produtoId, r.ultimaData);
        }
        ...
        const pedidosParados = await conn
            ...
            .from(db.pedidos)
            .where(db.and(db.eq(db.pedidos.tenantId, tenantIdNum), inArray(db.pedidos.status, [PedidoStatus.GERADO, PedidoStatus.PENDENTE_ESTOQUE]), db.sql `${db.pedidos.createdAt} < DATE_SUB(NOW(), INTERVAL 20 DAY)`))
            .limit(1);
        ...
        const [recebidoHoje] = await conn
            .select({ total: db.sql `COALESCE(SUM(${db.contasReceber.valor}), 0)` })
            .from(db.contasReceber)
            .where(db.and(db.eq(db.contasReceber.tenantId, tenantIdNum), db.eq(db.contasReceber.status, ContaReceberStatus.RECEBIDA), db.sql `DATE(${db.contasReceber.dataRecebimento}) = CURDATE()`));
        const [pagoHoje] = await conn
            .select({ total: db.sql `COALESCE(SUM(${db.contasPagar.valor}), 0)` })
            .from(db.contasPagar)
            .where(db.and(db.eq(db.contasPagar.tenantId, tenantIdNum), db.eq(db.contasPagar.status, ContaPagarStatus.PAGO), db.sql `DATE(${db.contasPagar.dataPagamento}) = CURDATE()`));
        const totalRecebido = Number(recebidoHoje?.[0]?.total ?? 0);
        const totalPago = Number(pagoHoje?.[0]?.total ?? 0);
        ...
        const contasReceberHoje = await conn
            ...
            .from(db.contasReceber)
            .where(db.and(db.eq(db.contasReceber.tenantId, tenantIdNum), db.eq(db.contasReceber.status, ContaReceberStatus.PENDENTE), db.sql `DATE(${db.contasReceber.dataVencimento}) = CURDATE()`))
            .limit(1);
        ...
        const countHoje = Number(pedidosHoje?.[0]?.c ?? 0);
        const count7 = Number(pedidosUltimos7?.[0]?.c ?? 0);
