...
async function validateRedisConnection() {
    try {
        ...
        logError("CACHE_ERROR", "Falha ao validar conexão Redis", { service: "cache-manager", error: error });
    }
}
/**
...
export function createCacheRouter() {
    ...
    router.post("/clear", requireAdmin, (req, res) => {
        ...
        logInfo("Cache limpo manualmente", {
            payload: { user: req.user?.name || "admin" }
        });
        ...
        try {
            ...
            if (pattern) {
                ...
                logInfo(`Cache invalidado por padrão: ${pattern}`, {
                    payload: { user: req.user?.name || "admin", count }
                });
            }
