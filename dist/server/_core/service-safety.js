 ...
 */
export function isObjectSafe(value) {
 ...
 */
export function hasValidId(value) {
    return isObjectSafe(value) && 'id' in value && typeof value.id === 'number';
}
/**
 ...
 */
export function ensureArray(result, context) {
 ...
 */
export function ensureObject(result, context) {
    const callerContext = getCallerContext();
    const service = context?.service || callerContext.service;
    const method = context?.method || callerContext.method;
    const traceId = context?.traceId || nanoid(10);
 ...
 */
export function ensureCreatedResult(result, context) {
    const callerContext = getCallerContext();
    const service = context?.service || callerContext.service;
    const method = context?.method || callerContext.method;
    const traceId = context?.traceId || nanoid(10);
 ...
 */
export function ensureUpdateResult(result, context) {
    const callerContext = getCallerContext();
    const service = context?.service || callerContext.service;
    const method = context?.method || callerContext.method;
    const traceId = context?.traceId || nanoid(10);
 ...
 * @returns Um objeto indicando sucesso
 */
export function ensureDeleteResult(result, context) {
    const callerContext = getCallerContext();
    const service = context?.service || callerContext.service;
    const method = context?.method || callerContext.method;
    const traceId = context?.traceId || nanoid(10);
 ...
 */
export function ensurePaginatedResult(items, total, page, pageSize, context) {
    return {
 ...
 */
export function EnsureArrayReturn(context) {
 ...
 * @returns Um decorator que envolve o método original com uma verificação de tipo
 */
export function EnsureCreatedResult(context) {
    return function (_target, propertyKey, descriptor) {
        const originalMethod = descriptor.value;
        descriptor.value = async function (...args) {
            const method = String(propertyKey);
 ...
 */
export function createSafeService(service, serviceName) {
    const safeService = {};
    for (const key of Object.keys(service)) {
        ...
        if (typeof originalMethod === 'function') {
            ...
            if (methodName.startsWith('get') || methodName.startsWith('list') || methodName.startsWith('find')) {
                ...
                safeService[key] = async function (...args) {
                    const traceId = nanoid(10);
                    try {
    ...
    return Object.assign({}, service, safeService);
}
