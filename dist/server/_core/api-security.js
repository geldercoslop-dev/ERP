...
function sendStandardError(req, res, status, code, message) {
    res.status(status).json({ error: code, message });
}
function logSecurity(message, req, details) {
...
export function apiSecurityMiddleware(req, res, next) {
    ...
    if (req.path === "/health" || req.path === "/health/")
        return next();
    if (req.path.includes("/trpc/auth.login"))
        return next();
    ...
    const hasBearerAuth = Array.isArray(authHeader)
 ...
 */
export function browserSecurityMiddleware(req, res, next) {
