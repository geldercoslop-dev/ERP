...
export const pedidoSchema = z.object({
    ...
    itens: z.array(pedidoItemSchema)
        ...
        .refine((items) => items.every(item => Number((item.quantidade * item.valorUnitario).toFixed(2)) === Number((item.quantidade * item.valorUnitario).toFixed(2))), 'Valores dos itens devem ter no máximo 2 casas decimais'),
    idempotencyKey: z.string()
        .max(64, 'Chave de idempotência deve ter no máximo 64 caracteres')
        .optional()
});
// Função para validar dados com log de erro
export function validateWithLog(schema, data, context) {
    try {
        ...
        if (error instanceof z.ZodError) {
            ...
            throw new ValidationError(`Validação falhou: ${errors.map((e) => `${e.field}: ${e.message}`).join(', ')}`);
        }
