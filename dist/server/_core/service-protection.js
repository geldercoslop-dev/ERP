 ...
 */
const protectedServices = new WeakMap();
/**
 ...
 */
export function incrementInvalidReturnCount() {
    invalidReturnCount++;
    const now = Date.now();
    ...
    if (now - lastCounterReset > COUNTER_RESET_INTERVAL) {
        ...
        if (invalidReturnCount >= INVALID_RETURN_THRESHOLD) {
            logCritical('CRITICAL_SYSTEM_ISSUE', `Detectados ${invalidReturnCount} retornos inválidos no último minuto`, { payload: { invalidReturnCount } });
        }
        ...
        logCritical('CRITICAL_SYSTEM_ISSUE', `Detectados ${invalidReturnCount} retornos inválidos em menos de um minuto`, { payload: { invalidReturnCount } });
    }
}
/**
 ...
 */
export function getInvalidReturnCount() {
 ...
 */
export function protectService(serviceName, service) {
 ...
 */
export function protectServiceModule(moduleName, moduleExports) {
    const protectedExports = { ...moduleExports };
    for (const key of Object.keys(moduleExports)) {
        ...
        if (typeof value === 'function' &&
            ...
            protectedExports[key] = function (...args) {
 ...
 * @param moduleName - O nome do módulo para fins de log
 */
export function applyServiceProtection(serviceModule, moduleName) {
    ...
    for (const key of keys) {
        try {
            ...
            if (!globalThis.__erpEsmProtectionWarned) {
                globalThis.__erpEsmProtectionWarned = true;
                logInfo("Proteção in-place de serviços desativada (namespace ESM read-only). Esperado em Node ESM.");
            }
            return;
        }
    }
