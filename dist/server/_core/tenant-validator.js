 ...
 */
export function validateTenantAccess(tenantId, actor) {
 ...
 */
export function validateQueryTenantIsolation(query) {
    // Verificar se a query menciona tenantId em cláusulas WHERE
    const hasTenantFilter = query.includes('tenantId') ||
 ...
 */
export function validateSecureQuery(query, tenantId) {
    if (!tenantId || tenantId <= 0) {
...
export class DatabaseSecurityAuditor {
     ...
     * @returns true se seguro, false se vulnerável
     */
    validateQuery(query, tenantId, context) {
        try {
            ...
            this.violations.push(`Erro na validação em ${context}: ${error.message}`);
            return false;
        }
    }
    /**
