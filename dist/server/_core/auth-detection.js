...
async function tableExists(connection, tableName) {
    try {
        const [rows] = await connection.execute(`SHOW TABLES LIKE '${tableName.replace(/'/g, "''")}'`);
...
async function getTableStructure(connection, tableName) {
    try {
        ...
        const tableRows = rows;
        return Array.isArray(tableRows) ? tableRows.map((row) => row.Field) : [];
    }
    catch (error) {
 ...
 */
function detectUserAuthFields(columns) {
    const usernameField = columns.find(col => ["username", "login", "email", "openId"].includes(col));
    const passwordField = columns.find(col => ["password", "senha", "pass"].includes(col));
    const emailField = columns.find(col => ["email", "email_address"].includes(col));
    const roleField = columns.find(col => ["role", "perfil", "user_role"].includes(col));
 ...
 */
function detectVendedorAuthFields(columns) {
    const usernameField = columns.find(col => ["usuario", "login", "email", "nome"].includes(col));
    const passwordField = columns.find(col => ["senha", "password", "pass"].includes(col));
    const emailField = columns.find(col => ["email", "email_address"].includes(col));
    if (usernameField && passwordField) {
        return {
            table: "vendedores",
 ...
 */
async function createAdminIfNeeded(connection, config) {
    try {
        console.log(`[Auth Detection] Verificando admin na tabela ${config.table}...`);
        // Verificar se admin já existe
        const [existingRows] = await connection.execute(`SELECT * FROM ${mysql.escapeId(config.table)} WHERE ${mysql.escapeId(config.usernameField)} = ? LIMIT 1`, ["admin"]);
        const existingRowsArray = existingRows;
 ...
 */
async function saveAuthConfigToFile(config) {
    try {
 ...
 */
async function loadAuthConfigFromFile() {
    try {
        const configPath = path.join(process.cwd(), ".auth-config.json");
...
export async function authenticateUser(username, password) {
    ...
    try {
        // Buscar usuário
        const [rows] = await connection.execute(`SELECT * FROM ${mysql.escapeId(config.table)} WHERE ${mysql.escapeId(config.usernameField)} = ? LIMIT 1`, [username]);
 ...
 */
export function resetAuthDetection() {
    authConfig = null;
