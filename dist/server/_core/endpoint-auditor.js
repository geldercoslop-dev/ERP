import { authLogger, dbLogger, apiLogger } from './logger.js';
/**
...
export class EndpointAuditor {
     ...
     */
    validateCriticalEndpointsCoverage(registeredEndpoints) {
        ...
        return {
            ...
            coveredCritical: CRITICAL_ENDPOINTS.filter(c => c.critical && registeredEndpoints.some(endpoint => endpoint.includes(c.endpoint))).length
        };
    }
}
 ...
 */
export function needsAdditionalAudit(endpoint) {
 ...
 */
export function getCriticalFields(endpoint) {
    const config = auditor.getAuditConfig(endpoint);
