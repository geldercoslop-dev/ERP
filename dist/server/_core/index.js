...
import { checkDatabasePoolHealth } from "../config/database.js";
...
function sendStandardError(req, res, status, code, message, details) {
...
function isResponseLocked(res) {
    const timedOut = res.locals.requestTimedOut === true;
    return timedOut || res.headersSent || res.writableEnded;
}
async function sendUnifiedHealthResponse(req, res) {
    console.log("STEP FINAL", req.originalUrl, req.url, req.path);
    console.log("[HEALTH-UNIFIED] PASSO 1: Iniciando sendUnifiedHealthResponse");
    res.setHeader("Cache-Control", "no-store");
    console.log("[HEALTH-UNIFIED] PASSO 2: Cache header definido");
    try {
        console.log("[HEALTH-UNIFIED] PASSO 3: Iniciando checks de DB e Redis");
        let dbUp = false;
        let redisUp = false;
        try {
            console.log("[HEALTH-UNIFIED] PASSO 4: Chamando checkDatabasePoolHealth");
            dbUp = await checkDatabasePoolHealth();
            console.log("[HEALTH-UNIFIED] PASSO 5: DB check concluído", { dbUp });
        }
        catch (error) {
            console.error("[HEALTH-UNIFIED] ERRO DB:", error instanceof Error ? error.message : String(error));
            dbUp = false;
        }
        try {
            console.log("[HEALTH-UNIFIED] PASSO 6: Verificando Redis");
            const redis = getRedisClient();
            const pong = redis ? await redis.ping() : "";
            redisUp = pong === "PONG";
            console.log("[HEALTH-UNIFIED] PASSO 7: Redis check concluído", { redisUp, pong });
        }
        catch (error) {
            console.error("[HEALTH-UNIFIED] ERRO REDIS:", error instanceof Error ? error.message : String(error));
            redisUp = false;
        }
        const ok = dbUp && redisUp;
        console.log("[HEALTH-UNIFIED] PASSO 8: Status final determinado", { ok, dbUp, redisUp });
        if (isResponseLocked(res)) {
            console.log("[HEALTH-UNIFIED] AVISO: Resposta bloqueada por timeout");
            systemLogger.warn({ path: req.path, requestId: getRequestId(req) }, "[HEALTH] resposta suprimida por timeout/resposta encerrada");
            return;
        }
        console.log("[HEALTH-UNIFIED] PASSO 9: Enviando response JSON");
        const responseData = {
            status: ok ? "ok" : "error",
            db: dbUp ? "ok" : "error",
            redis: redisUp ? "ok" : "error",
        };
        console.log("[HEALTH-UNIFIED] PASSO 9.5: Response data preparada", { responseData });
        console.log("ANTES DO SEND - ENVIANDO RESPOSTA");
        // BYPASS TOTAL DO res.json - TESTE DE PROVA
        res.status(200).json(responseData);
        console.log("DEPOIS DO SEND - RESPOSTA ENVIADA");
        console.log("[HEALTH-UNIFIED] PASSO 10: Response enviada com sucesso");
        console.log("[HEALTH-UNIFIED] PASSO 11: Verificando se fluxo continua após res.end()");
        return;
    }
    catch (error) {
        console.error("[HEALTH-UNIFIED] ERRO GERAL:", {
        ...
        if (isResponseLocked(res)) {
            console.log("[HEALTH-UNIFIED] AVISO: Resposta bloqueada no catch");
            return;
        }
        console.log("[HEALTH-UNIFIED] PASSO 11: Enviando response de erro");
        res.status(503).json({
            status: "error",
        });
        console.log("[HEALTH-UNIFIED] PASSO 12: Response de erro enviada");
    }
}
...
export async function startServer() {
    ...
    console.log("ENV DEBUG:", {
        ...
        REDIS_HOST: parseEnv().REDIS_HOST,
        JWT_SECRET: parseEnv().JWT_SECRET ? "OK" : "MISSING",
    });
    ...
    // initializeOpenTelemetry(); // DESABILITADO PARA TESTE CIRÚRGICO
    ...
    try {
        const timeoutMs = Math.max(5_000, Number(parseEnv().REDIS_BOOT_TIMEOUT_MS || 30_000));
    ...
    // LOG STEP 1 - ANTES DE TUDO
    app.use((req, res, next) => {
        console.log("STEP 1", req.originalUrl, req.url, req.path);
        next();
    });
    // DEBUG MIDDLEWARE - Log todas as requisições
    app.use((req, res, next) => {
        console.log('[DEBUG] REQUISICAO RECEBIDA:', {
            method: req.method,
            url: req.url,
            path: req.path,
    ...
    // app.use(requestIdMiddleware); // DESABILITADO PARA TESTE CIRÚRGICO
    // app.use(requestLoggerMiddleware()); // TEMPORARIAMENTE DESABILITADO PARA TESTE
    // systemLogger.info("[BOOT] monitoramento…");
    // setupMonitoring(app); // DESABILITADO PARA TESTE CIRÚRGICO
    ...
    try {
        ...
        systemLogger.error("[ERROR] ❌ ensureAdminUser falhou - FAIL-HARD");
        // FAIL-HARD: admin user falhou → derrubar sistema
        throw new InfrastructureError(`Admin user initialization failed: ${e instanceof Error ? e.message : String(e)}`);
    }
    systemLogger.info("[BOOT] middlewares de segurança…");
    // STEP 2 - LOG ANTES DOS MIDDLEWARES CRÍTICOS
    app.use((req, res, next) => {
        console.log("STEP 2 - BEFORE SECURITY", req.originalUrl, req.url, req.path);
        next();
    });
    // Request ID primeiro (para tracking)
    // app.use(requestIDMiddleware); // DESABILITADO PARA TESTE CIRÚRGICO
    // Timeout e timing - TEMPORARIAMENTE DESABILITADO
    // app.use(timeoutMiddleware);
    // app.use(timingMiddleware); // DESABILITADO PARA TESTE CIRÚRGICO
    // Rate limiting - TEMPORARIAMENTE DESABILITADO
    // app.use(rateLimitMiddleware);
    // Security headers e validação - TEMPORARIAMENTE DESABILITADO
    // app.use(securityMiddleware);
    // Request ID legado (compatibilidade)
    // app.use(requestIdMiddleware); // DESABILITADO PARA TESTE CIRÚRGICO
    // app.use(requestLoggerMiddleware()); // TEMPORARIAMENTE DESABILITADO
    // Security headers existente - TEMPORARIAMENTE DESABILITADO
    // app.use(securityHeadersMiddleware());
    // HTTP Hardening - Rate Limit Global + Security Headers + Trust Proxy - TEMPORARIAMENTE DESABILITADO
    // const hardeningMiddlewares = httpHardeningMiddleware();
    // hardeningMiddlewares.forEach(middleware => app.use(middleware));
    // Strict health check (garante 503 em falhas) - TEMPORARIAMENTE DESABILITADO
    // app.use(strictHealthMiddleware);
    const allowedOrigins = (parseEnv().ALLOWED_ORIGINS ||
        "http://localhost:5173,http://127.0.0.1:5173")
        .split(",")
        .map((o) => o.trim())
        .filter(Boolean);
    // STEP 2 - LOG ANTES DO CORS
    app.use((req, res, next) => {
        console.log("STEP 2 - BEFORE CORS", req.originalUrl, req.url, req.path);
        next();
    });
    app.use(cors({
        origin: (origin, cb) => {
    ...
    // STEP 2 - LOG ANTES DO BODY PARSER
    app.use((req, res, next) => {
        console.log("STEP 2 - BEFORE BODY PARSER", req.originalUrl, req.url, req.path);
        next();
    });
    app.use(express.json({ limit: "1mb" }));
    app.use(express.urlencoded({ limit: "1mb", extended: true }));
    app.use(cookieParser());
    // STEP 2 - LOG ANTES DO SECURITY MIDDLEWARE
    app.use((req, res, next) => {
        console.log("STEP 2 - BEFORE API SECURITY", req.originalUrl, req.url, req.path);
        next();
    });
    ...
    const internalEndpointSecret = process.env.HARD_TEST_SHUTDOWN_SECRET?.trim() || appSecret || "";
    app.use((req, res, next) => {
        ...
        if (req.path.includes("/api/trpc/auth.login"))
            return next();
        if (req.path.includes("/api/csrf-token"))
            return next();
        ...
        for (const pattern of MALICIOUS_PATTERNS) {
            if (pattern.test(inspectionTargets)) {
                ...
                return sendStandardError(req, res, 400, "SUSPICIOUS_PAYLOAD", "suspicious payload blocked");
            }
        }
        next();
    });
    app.use((req, res, next) => {
        if (!req.path.startsWith("/api/__"))
            return next();
    ...
    if (process.env.HARD_TEST_HTTP_SHUTDOWN === "1") {
        const shutdownSecret = process.env.HARD_TEST_SHUTDOWN_SECRET?.trim() || "erp-hard-test-local-secret";
        ...
        app.get("/api/__hard-test/shutdown", shutdownRateLimit, (req, res) => {
            ...
            if (!allowedOrigins.includes(clientIP) && !clientIP?.startsWith('192.168.') && !clientIP?.startsWith('10.')) {
            ...
            setImmediate(async () => {
                try {
                    ...
                    securityLogger.error("[SECURITY] shutdown endpoint error", err, {
                        requestId: getRequestId(req),
                        path: req.originalUrl || req.url,
                        method: req.method,
                        metadata: {
        ...
        console.warn("[HARD_TEST] GET /api/__hard-test/shutdown habilitado com proteções (defina HARD_TEST_SHUTDOWN_SECRET forte em CI)");
    }
    ...
    // Health check: único endpoint canônico /api/health - MOVIDO PARA DEPOIS DOS MIDDLEWARES
    app.get("/api/health", sendUnifiedHealthResponse);
    ...
    app.get("/api/backup/download", (req, res, next) => {
        ...
        try {
            const tenantId = String(req.adminTenantId ?? "");
    ...
    console.log("[BOOT] PORT configurada:", PORT);
    console.log("[BOOT] iniciando servidor na porta", PORT);
    // FAIL-HARD: Verificar se porta está disponível sem fallback automático
    if (!(await isPortAvailable(PORT))) {
        console.error(`[BOOT][FATAL] porta ${PORT} em uso`);
        console.error(`[BOOT][FATAL] encerrando por fail-hard de porta`);
        process.exit(1);
    }
    ...
    const serverInstance = server.listen(PORT, () => {
        ...
        const realPort = typeof address === 'object' && address !== null ? address.port : PORT;
        systemLogger.info(`[BOOT] servidor ouvindo em http://localhost:${realPort}/`);
        global.SERVER_PORT = realPort;
    });
}
...
if (process.platform === "win32") {
    import("readline")
        ...
        .catch((err) => {
        systemLogger.error({ error: err instanceof Error ? err.message : String(err) }, "[SHUTDOWN] Falha ao carregar readline");
    });
}
startServer().catch((e) => {
    systemLogger.error({ error: e instanceof Error ? e.message : String(e) }, "[BOOT] startServer falhou");
