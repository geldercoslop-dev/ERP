...
function getMapsConfig() {
    ...
    if (!baseUrl || !apiKey) {
        throw new InfrastructureError("Google Maps proxy credentials missing: set BUILT_IN_FORGE_API_URL and BUILT_IN_FORGE_API_KEY");
    }
    return {
...
export async function makeRequest(endpoint, params = {}, options = {}) {
    ...
    if (!response.ok) {
        ...
        throw new InfrastructureError(`Google Maps API request failed (${response.status} ${response.statusText}): ${errorText}`);
    }
