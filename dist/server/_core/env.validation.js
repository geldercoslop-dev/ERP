...
function getTrimmedString(env, key) {
    const value = env[key];
    return typeof value === "string" ? value.trim() : "";
}
function isLongEnough(value, min) {
...
export function validateProductionEnv() {
    ...
    if (base) {
        ...
        if (!isRequired(base.DB_HOST))
            issues.push({ key: "DB_HOST", message: "is required" });
        if (!isRequired(base.DB_PORT))
            issues.push({ key: "DB_PORT", message: "is required" });
        if (!isRequired(base.DB_USER))
            issues.push({ key: "DB_USER", message: "is required" });
        if (!isRequired(base.DB_PASSWORD))
            issues.push({ key: "DB_PASSWORD", message: "is required" });
        if (!isRequired(base.DB_NAME))
            issues.push({ key: "DB_NAME", message: "is required" });
...
export function validateCriticalBootEnvOrExit(env) {
    const issues = [];
...
export function validateProductionEnvOrExit() {
