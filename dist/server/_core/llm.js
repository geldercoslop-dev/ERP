...
const normalizeToolChoice = (toolChoice, tools) => {
    ...
    if (toolChoice === "required") {
        ...
        if (tools.length > 1) {
            throw new ValidationError("tool_choice 'required' needs a single tool or specify the tool name explicitly");
        }
        return {
            type: "function",
...
export async function invokeLLM(params) {
    ...
    if (!response.ok) {
        ...
        throw new InfrastructureError(`LLM invoke failed: ${response.status} ${response.statusText} – ${errorText}`);
    }
