...
export async function enviarMensagemWhatsApp(numero, mensagem) {
    ...
    if (zapi) {
        try {
            const res = await fetch(`https://api.z-api.io/instances/${zapi.instance}/token/${zapi.token}/send-text`, {
                method: "POST",
            ...
            if (!res.ok)
                return { ok: false, erro: data?.message ?? data?.error ?? "Falha ao enviar." };
            return { ok: true };
        }
        catch (e) {
    ...
    if (evolution) {
        try {
            ...
            if (!res.ok)
                return { ok: false, erro: "Evolution API: falha ao enviar." };
            return { ok: true };
        }
        catch (e) {
...
export async function enviarBoletoWhatsApp(numero, texto, urlBoleto) {
    const msg = urlBoleto ? `${texto}\n\n${urlBoleto}` : texto;
    return enviarMensagemWhatsApp(numero, msg);
}
/** Envia status de entrega (texto livre). */
export async function enviarStatusEntrega(numero, pedidoRef, status, detalhe) {
