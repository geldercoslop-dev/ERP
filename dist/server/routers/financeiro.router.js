...
export const boletosRouter = router({
    ...
    gerarBoletosCarga: protectedProcedure
        ...
        if (ctx.user.role !== "admin") {
            ...
            const cargaData = carga;
            const pedidosIds = Array.isArray(cargaData.pedidos)
                ? cargaData.pedidos.map((p) => p.id)
                : [];
    ...
    create: protectedProcedure
        ...
        try {
            const result = await executeCommand({ commandName: "contasReceber.create", idempotencyKey: input.idempotencyKey }, async (tx) => {
                ...
                await financeService.createContaReceber(tenantId, {
...
export const planoContasRouter = router({
    ...
    update: adminProcedure
        ...
        const result = await executeCommand({ commandName: "planoContas.update", idempotencyKey: idempotencyKey ?? undefined }, async () => {
    ...
    delete: adminProcedure
        ...
        const tenantId = ctx.tenantId;
        assertTenantId(tenantId);
        const result = await executeCommand({ commandName: "planoContas.delete", idempotencyKey: input.idempotencyKey ?? undefined }, async () => {
