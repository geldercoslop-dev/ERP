...
export const clientesRouter = router({
    ...
    getHistorico: protectedProcedure
        ...
        if (!result.success)
            throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: result.error ?? "Falha ao buscar histórico" });
        return result.data ?? [];
    }),
    ...
    vinculate: protectedProcedure
        ...
        await clientesService.associarClienteVendedor(tenantId, input.clienteId, vendedor.id, input.tipo === "PRINCIPAL");
        return { success: true };
    }),
