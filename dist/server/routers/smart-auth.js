        ...
        try {
            ...
            if (authConfig.table === "users") {
                ...
                if (userObj.tenantId && userObj.id) {
                    await usersService.touchLastSignedIn(userObj.tenantId, userObj.id);
                }
            }
            else {
                // Sessão de vendedor
                ...
                name = userObj.nome || userObj.name || "Vendedor";
                // Atualizar último signin
                if (userObj.tenantId && userObj.userId) {
                    await usersService.touchLastSignedIn(userObj.tenantId, userObj.userId);
                }
            }
            // Definir cookies
