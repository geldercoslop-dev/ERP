...
export class OrderTool {
    ...
    async create(input) {
        ...
        try {
            ...
            const serviceResult = hasSuccess ? result : { success: true, data: result };
    ...
    enrichCreatePayload(input) {
        ...
        if (enriched.clienteId === undefined &&
        ...
        if (!('total' in enriched) && Array.isArray(enriched.itens)) {
            const total = enriched.itens.reduce((sum, item) => {
                if (item && typeof item === 'object' &&
    ...
    normalizeListPayload(input) {
        ...
        if (normalized.clienteId === undefined &&
