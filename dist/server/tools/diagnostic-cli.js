...
class DiagnosticCLI {
     ...
     */
    printDiagnosticReport(diagnostic, verbose = false) {
        ...
        if (diagnostic.databaseHealth.slowQueries.length > 0) {
            ...
            if (verbose) {
                ...
                diagnostic.databaseHealth.slowQueries.slice(0, 3).forEach((query, index) => {
                    const q = query;
        ...
        if (diagnostic.routeHealth.errors.length > 0) {
            ...
            if (verbose) {
                ...
                diagnostic.routeHealth.errors.slice(0, 3).forEach((error, index) => {
                    const e = error;
        ...
        const serviceEntries = Object.entries(diagnostic.serviceHealth);
     ...
     */
    printRecommendations(diagnostic) {
        ...
        // Serviços
        const errorServices = Object.entries(diagnostic.serviceHealth || {});
