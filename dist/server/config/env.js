...
export function validateEnv() {
    try {
        ...
        if (process.env.NODE_ENV === 'development') {
            ...
            console.log(`[ENV] Database: ${validatedEnv.DATABASE_HOST ?? '(via DATABASE_URL)'}:${validatedEnv.DATABASE_PORT ?? ''}/${validatedEnv.DATABASE_NAME ?? ''}`);
        }
        return validatedEnv;
    }
    catch (error) {
 ...
 */
export function validateJwtSecretStrength(secret, context = 'JWT_SECRET') {
 ...
 */
export function getRedisOptions(env) {
    return {
        host: env.REDIS_HOST,
        port: env.REDIS_PORT,
