 ...
 */
export function requireValidEnv(req, res, next) {
    try {
        ...
        logger.error('Invalid environment in request', error);
        res.status(500).json({
...
export function checkEnvironmentHealth() {
    ...
    try {
        ...
        if (!env.JWT_ACCESS_SECRET)
            issues.push('JWT_ACCESS_SECRET missing');
        if (!env.JWT_REFRESH_SECRET)
            issues.push('JWT_REFRESH_SECRET missing');
        if (env.JWT_ACCESS_SECRET.length < ENV_SECRET_MIN_LENGTH)
            issues.push('JWT_ACCESS_SECRET too short');
        if (env.JWT_REFRESH_SECRET.length < ENV_SECRET_MIN_LENGTH)
            issues.push('JWT_REFRESH_SECRET too short');
        return {
