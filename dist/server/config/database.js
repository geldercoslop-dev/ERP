...
export function requireDatabaseUrl() {
    ...
    if (!raw) {
        throw new InfrastructureError("DATABASE_URL é obrigatório. Ex.: mysql://usuario:senha@host:3306/nome_do_banco");
    }
    return raw;
}
function parseUrlToPoolOptions(urlString) {
    let u;
    try {
        u = new URL(urlString);
    }
    ...
    const connectionLimit = Math.max(10, Number(process.env.DB_POOL_CONNECTION_LIMIT || process.env.MYSQL_POOL_SIZE || 20));
...
export function getPoolHealthSnapshot() {
 ...
 */
export async function checkDatabasePoolHealth() {
    const now = Date.now();
...
function getDatabaseConfig() {
    ...
    console.log("DB CONFIG:", {
        ...
        envDbUrl: parseEnv().DATABASE_URL ? "SET" : "NOT_SET",
        envDbHost: parseEnv().DB_HOST,
        envDbPort: parseEnv().DB_PORT,
        envDbUser: parseEnv().DB_USER,
        envDbName: parseEnv().DB_NAME
    });
    return config;
}
/**
 ...
 */
export function clearPoolAfterGracefulShutdown() {
    _pool = null;
    globalThis.db = undefined;
}
async function testPool(pool, maxRetries = 10) {
    let lastError;
    ...
    const lastCode = lastError && typeof lastError === "object" && "code" in lastError ? lastError.code : undefined;
    throw new InfrastructureError(`Failed to connect to database after ${maxRetries} attempts: ${lastMsg} (code: ${lastCode})`);
}
export async function getConnection(maxRetries = 10) {
    const pool = await getConnectionPool();
    let lastError;
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
...
export async function executeQuery(query, params = [], maxRetries = 3) {
    ...
    return executeWithResilience(async () => _executeQueryCore(query, params), {
 ...
 */
async function _executeQueryCore(query, params = []) {
    ...
    try {
        ...
        observabilityLogger.error("Database query failed", error instanceof Error ? error : new Error(msg), {
            duration,
            query: query.replace(/\s+/g, " ").substring(0, 200),
        });
        throw error;
