 ...
 */
export function verificarConfiguracaoIntegracoes() {
    const configuradas = [];
    const naoConfiguradas = [];
    // OpenWeather (clima)
    ...
    if (temValor(telegramToken) && temValor(telegramChat)) {
        ...
        const faltando = [];
        if (!temValor(telegramToken))
            faltando.push("TELEGRAM_BOT_TOKEN");
        if (!temValor(telegramChat))
            faltando.push("TELEGRAM_CHAT_ID");
        naoConfiguradas.push({
            nome: "Telegram (notificações)",
            status: "nao_configurada",
            variaveis: ["TELEGRAM_BOT_TOKEN", "TELEGRAM_CHAT_ID"],
