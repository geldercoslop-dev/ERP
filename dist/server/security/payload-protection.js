...
export function payloadLimitMiddleware(options = {}) {
    const { jsonLimit = '1mb', urlencodedLimit = '1mb', textLimit = '1mb', rawLimit = '1mb', } = options;
    return [
...
export function endpointPayloadLimit(options) {
    const { maxBodySize = 1024 * 1024, // 1MB padrão
    maxParamCount = 100, sensitivePaths = ['/auth', '/login', '/leo/'], } = options;
    return (req, res, next) => {
        try {
...
export function fileUploadMiddleware(options) {
    ...
    maxFiles = 5, allowedTypes = ['image/jpeg', 'image/png', 'image/gif', 'application/pdf'], } = options;
    return (req, res, next) => {
