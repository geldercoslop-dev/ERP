...
function defaultKeySuffix(req) {
    const endpoint = (req.path || req.url || "unknown").split("?")[0];
    return `${normalizeIp(req)}:${endpoint}`;
}
export function createRedisRateLimitMiddleware(options) {
    const { name, windowMs, max, code, message, shouldApply, keySuffix = defaultKeySuffix, onBlocked, } = options;
    return async (req, res, next) => {
        ...
        try {
            ...
            const now = Date.now();
            const raw = (await redis.eval(LUA_FIXED_WINDOW, 1, key, String(now), String(windowMs), String(max)));
