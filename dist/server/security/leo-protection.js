...
class LeoProtection {
    ...
    static WINDOW_MS = 60 * 1000; // 1 minuto
    static MAX_REQUESTS = 20;
    static BLOCK_DURATION_MS = 5 * 60 * 1000; // 5 minutos
    static CLEANUP_INTERVAL = 10 * 60 * 1000; // 10 minutos
    /**
     * Obtém chave para o tracker
     */
    static getKey(tenantId, userId, ip) {
        return `leo:${tenantId}:${userId || 'anonymous'}:${ip || 'unknown'}`;
    }
    /**
     * Verifica se o usuário pode fazer requisição
     */
    static canMakeRequest(tenantId, userId, ip) {
     ...
     */
    static detectRapidRequests(tenantId, userId, ip) {
     ...
     */
    static detectAnomalousRequests(req) {
 ...
 */
export function getLeoStats(req, res) {
    try {
        ...
        logger.error('Error getting LEO stats', error);
        res.status(500).json({
            success: false,
 ...
 */
export function resetLeoStats(req, res) {
    try {
        // Verifica se é admin
        const user = req.user;
        ...
        logger.error('Error resetting LEO stats', error);
        res.status(500).json({
            success: false,
