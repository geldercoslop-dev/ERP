...
class LeoSupervisor {
    ...
    executionTimes = [];
    isPausedUntil; // Novo: timestamp de fim da pausa
    pauseCount = 0; // Contador de pausas
    constructor() {
     ...
     */
    async completeAction(taskId, executionTime, success) {
        try {
     ...
     */
    async pauseExecution(reason, durationMinutes) {
        try {
     ...
     */
    async handleViolation(type, value, limit) {
        try {
            const violation = {
                type,
                timestamp: new Date(),
     ...
     */
    async handleFailure(taskId, executionTime) {
        try {
    ...
    recordAction() {
        ...
        this.actionTimestamps = this.actionTimestamps.filter((timestamp) => timestamp > oneMinuteAgo);
