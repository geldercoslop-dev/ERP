...
//   return tenantId;
// }
async function verificarVendasDoDia(req) {
    try {
        if (!req.user) {
        ...
        return { nome: "vendas_do_dia", ok: false, erro: e?.message ?? String(e) };
    }
}
async function verificarEstoqueBaixo(req) {
    try {
        if (!req.user) {
            throw new ValidationError("Usuário não autenticado");
        }
        ...
        return { nome: "estoque_baixo", ok: false, erro: e?.message ?? String(e) };
    }
}
async function verificarBoletosVencidos(req) {
    try {
        if (!req.user) {
            throw new ValidationError("Usuário não autenticado");
        }
        ...
        return { nome: "boletos_vencidos", ok: false, erro: e?.message ?? String(e) };
    }
}
async function verificarPedidosParados(req) {
    try {
        if (!req.user) {
            throw new ValidationError("Usuário não autenticado");
        }
        ...
        return { nome: "pedidos_parados", ok: false, erro: e?.message ?? String(e) };
    }
}
export async function executarTarefas(req) {
    const results = [];
...
let intervalId = null;
export function startScheduler(req) {
    if (intervalId != null)
        return;
    executarTarefas(req)
        .then((r) => {
    ...
    intervalId = setInterval(() => {
        executarTarefas(req)
            ...
            .catch((e) => console.error("[LEO scheduler]", e?.message ?? e));
    }, INTERVAL_MS);
}
