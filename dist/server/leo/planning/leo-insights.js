...
class LeoInsights {
     ...
     */
    async resolverInsight(insightId, resolucao, responsavel) {
        try {
     ...
     * Ignora um insight
     */
    async ignorarInsight(insightId) {
        try {
            const insight = this.insightsAtivos.get(insightId);
            if (!insight) {
                return {
    ...
    async getEstatisticas() {
        ...
        const porTipo = {
            oportunidade: insights.filter((i) => i.tipo === 'oportunidade').length,
            risco: insights.filter((i) => i.tipo === 'risco').length,
            tendencia: insights.filter((i) => i.tipo === 'tendencia').length,
            recomendacao: insights.filter((i) => i.tipo === 'recomendacao').length,
            alerta: insights.filter((i) => i.tipo === 'alerta').length,
        };
        const porImpacto = {
            critico: insights.filter((i) => i.impacto === 'critico').length,
            alto: insights.filter((i) => i.impacto === 'alto').length,
            medio: insights.filter((i) => i.impacto === 'medio').length,
            baixo: insights.filter((i) => i.impacto === 'baixo').length,
        };
        const porUrgencia = {
            critica: insights.filter((i) => i.urgencia === 'critica').length,
            alto: insights.filter((i) => i.urgencia === 'alta').length,
            media: insights.filter((i) => i.urgencia === 'media').length,
            baixa: insights.filter((i) => i.urgencia === 'baixa').length,
        };
        return {
            total: insights.length,
            ativos: insights.filter((i) => i.status === 'ativo').length,
            resolvidos: insights.filter((i) => i.status === 'resolvido').length,
            ignorados: insights.filter((i) => i.status === 'ignorado').length,
            porTipo,
            porImpacto,
            porUrgencia,
        };
