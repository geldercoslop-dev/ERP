 ...
 */
export async function enviarNotificacoesInteligentes(req, tarefasResumos) {
    ...
    if (boletoTask?.resumo)
        linhas.push(`📋 ${boletoTask.resumo}`);
    const estoqueTask = tarefasResumos.find((t) => t.nome === "estoque_baixo" && t.resumo && !t.resumo.startsWith("Nenhum"));
    if (estoqueTask?.resumo)
        linhas.push(`📦 ${estoqueTask.resumo}`);
    try {
        if (!req.user) {
        ...
        if (zero > 0)
            linhas.push(`⚠️ Estoque zerado: ${zero} produto(s) com 0 un.`);
    }
    catch {
        void 0;
    }
    try {
        ...
        if (p)
            linhas.push(`💰 Pedido grande registrado: #${p.numero ?? "?"} — R$ ${Number(p.total ?? 0).toFixed(2)}`);
    }
    catch {
        void 0;
    }
    ...
    if (paradosTask?.resumo)
        linhas.push(`📌 ${paradosTask.resumo}`);
    if (linhas.length === 0)
        return;
