...
export function checkLeoSecurity(userId, action, entity, parameters, context) {
    // Buscar regra de segurança
    const rule = LEO_SECURITY_RULES.find(r => (r.action === action || r.action === '*') &&
 ...
 */
function validateSecurityRestriction(restriction, parameters, entity, context) {
 ...
 */
function auditSecurityAction(userId, action, entity, parameters, ruleId, allowed) {
    const auditEntry = {
 ...
 */
export function isHighRiskAction(action, entity) {
    const rule = LEO_SECURITY_RULES.find(r => (r.action === action || r.action === '*') &&
        (r.entity === entity || r.entity === '*'));
 ...
 */
export async function checkLeoPermission(usuario, tipoAcao, entidade) {
    try {
 ...
 */
async function getUserPermission(usuario) {
    try {
 ...
 */
export function temPermissaoEspecifica(nivel, permissao) {
 ...
 */
export function getNivelMinimoParaAcao(tipoAcao, entidade) {
 ...
 */
export async function promoverUsuario(solicitante, usuarioAlvo, novoNivel) {
    try {
