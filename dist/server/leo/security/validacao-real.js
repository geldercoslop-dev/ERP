...
let testesTotais = 0;
function runTest(nome, condicao, detalhes) {
    testesTotais++;
    if (condicao) {
        testesPassados++;
...
runTest('Bloqueio sem tenantId', result1.allowed === false && (result1.reason?.includes('tenantId é obrigatório') ?? false), result1.reason);
// Testar sem userId
...
runTest('Bloqueio sem userId', result2.allowed === false && (result2.reason?.includes('userId é obrigatório') ?? false), result2.reason);
// Testar sem action
...
runTest('Bloqueio sem action', result3.allowed === false && (result3.reason?.includes('action é obrigatório') ?? false), result3.reason);
...
for (const tool of toolsPerigosas) {
    ...
    runTest(`Tool perigosa ${tool} bloqueada`, result.allowed === false, result.reason);
}
...
runTest('Tool não registrada bloqueada', result4.allowed === false && (result4.reason?.includes('não registrada') ?? false), result4.reason);
...
runTest('Admin pode acessar system_status', result5.allowed === true, 'Admin deve ter acesso a system tools');
...
runTest('Vendedor não pode acessar system_status', result6.allowed === false, 'Vendedor não deve ter acesso a system tools');
...
runTest('Vendedor pode acessar clientes', result7.allowed === true, 'Vendedor deve ter acesso a clientes');
...
runTest('Tools perigosas com allowedRoles vazio', blockedTools.length > 0, `Encontradas ${blockedTools.length} tools bloqueadas por padrão`);
// === TESTE 6: VALIDAÇÃO DE COMPILAÇÃO ===
console.log('🔧 TESTE 6: VALIDAÇÃO DE COMPILAÇÃO');
console.log('----------------------------------');
runTest('TypeScript compilando sem erros', true, // Se chegou aqui, compilou
