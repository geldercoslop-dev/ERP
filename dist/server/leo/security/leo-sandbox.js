...
class LeoSandbox {
    ...
    async checkAction(action) {
        ...
        const rule = this.rules.find(r => r.action === action.action || r.action === action.type);
        if (rule) {
            if (!rule.allowed) {
     ...
     */
    async checkShellAction(action) {
        const command = action.parameters?.command || action.action;
        ...
        const isDangerous = dangerousCommands.some(cmd => commandLower.includes(cmd));
        if (isDangerous) {
            return {
                allowed: false,
     ...
     */
    async checkDatabaseAction(action) {
        ...
        const isDangerous = dangerousOperations.some(op => operationUpper.includes(op));
        if (isDangerous) {
            return {
                allowed: false,
     ...
     */
    async checkFileAccessAction(action) {
        ...
        const isBlockedPath = this.blockedPaths.some(blockedPath => normalizedPath.toLowerCase().startsWith(blockedPath.toLowerCase()));
        if (isBlockedPath) {
            return {
                allowed: false,
     ...
     */
    async checkFinancialAction(action) {
        const operation = action.parameters?.operation || action.action;
        ...
        const isDangerous = dangerousOperations.some(op => operationLower.includes(op));
        if (isDangerous) {
            return {
                allowed: false,
     ...
     */
    async checkAutomationAction(action) {
        const params = (action.parameters && typeof action.parameters === 'object') ? action.parameters : {};
        ...
        const isCritical = criticalAutomations.some(auto => automationType.toLowerCase().includes(auto));
        if (isCritical) {
            return {
                allowed: false,
     ...
     */
    async executeAllowedAction(action) {
        switch (action.type) {
            case 'shell':
     ...
     */
    async executeShellAction(action) {
        const command = action.parameters?.command || action.action;
     ...
     */
    async executeFileAccessAction(action) {
        const filePath = action.parameters?.path || action.action;
        const operation = action.parameters?.operation || 'read';
        switch (operation) {
            case 'read':
     ...
     */
    isApplicationAllowed(appName) {
        return this.allowedApplications.some(allowed => appName.toLowerCase().includes(allowed.toLowerCase()));
    }
    /**
