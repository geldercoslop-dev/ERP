...
class LeoLoopProtection {
    ...
    consecutiveActions = new Map(); // Novo: contador de ações consecutivas
    alerts = [];
    monitoringInterval;
    constructor() {
     ...
     */
    async registerTaskStart(taskId, taskType) {
        try {
     ...
     */
    async registerTaskEnd(taskId, success, duration, error) {
        try {
     ...
     */
    cleanOldAlerts(maxAge = 24 * 60 * 60 * 1000) {
     ...
     */
    async blockTask(taskId, reason, durationMinutes = 30) {
        const history = this.taskHistory.get(taskId);
        if (!history)
            return;
     ...
     */
    getOrCreateHistory(taskId, taskType) {
     ...
     */
    detectSuspiciousPattern(history) {
        const now = Date.now();
     ...
     */
    async handleSuspiciousPattern(taskId, taskType, pattern) {
        const alert = {
     ...
     */
    async handleTaskFailure(taskId, history, error) {
     ...
     */
    async handleLongExecution(taskId, history, duration) {
        const alert = {
            id: `alert_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
            type: 'execution_loop',
            severity: 'medium',
     ...
     */
    async handleActionLoop(action, count) {
        const alert = {
            id: `alert_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
            type: 'action_loop',
            severity: 'high',
     ...
     */
    async handleConsecutiveActionLoop(action, count) {
    ...
    getStatistics() {
        ...
        const averageExecutions = tasks.length > 0
            ? tasks.reduce((sum, t) => sum + t.executions.length, 0) / tasks.length
            : 0;
        return {
