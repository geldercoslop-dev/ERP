...
export class LeoSalesAnalysis {
    ...
    async calculateAverageTicket(tenantId, vendedorId, period) {
    ...
    async analyzeSalesGrowth(tenantId, period = '30d') {
        try {
            ...
            const currentTotal = await ordersService.sumPedidosTotalBetween(tenantId, currentPeriodStart, new Date());
            const previousTotal = await ordersService.sumPedidosTotalBetween(tenantId, previousPeriodStart, currentPeriodStart);
            let percentage = 0;
            let trend = 'stable';
            if (previousTotal > 0) {
