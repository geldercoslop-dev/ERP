...
export const metricsService = {
    recordRequest(duration) {
        ...
        metricsStore.history.requests = metricsStore.history.requests.filter((t) => t > oneMinuteAgo);
    ...
    recordError() {
        ...
        const now = Date.now();
        const oneMinuteAgo = now - 60000;
        metricsStore.history.errors = metricsStore.history.errors.filter((t) => t > oneMinuteAgo);
    },
    recordConnection(increment) {
        ...
        return {
            ...
            averageResponseTime: responseTimes.length > 0
                ? (responseTimes.reduce((a, b) => a + b, 0) /
                    responseTimes.length).toFixed(2)
                : 0,
