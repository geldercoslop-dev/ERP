...
import { listPromocoes, createPromocao, updatePromocao, deletePromocao, getPromocaoItens, setPromocaoItens, } from '../services/promocoes.service.js';
import { requireTenantFromRequest } from '../_core/tenant-utils.js';
export async function listar(req) {
    try {
...
export async function detalhes(request) {
    try {
...
export async function criar(request) {
    try {
        const tenantId = requireTenantFromRequest(request);
        const body = z.object({
            nome: z.string().min(2),
...
export async function atualizar(request) {
    try {
        const tenantId = requireTenantFromRequest(request);
        const { id } = request.params;
        const body = z.object({
            ...
            ativo: z.boolean().optional(),
            itens: z.array(z.object({ produtoId: z.number().min(1), precoPromocional: z.coerce.number().min(0) })).optional(),
        }).parse(request.body);
