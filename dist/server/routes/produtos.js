...
import { createProduto as dbCreateProduto, updateProduto as dbUpdateProduto, deleteProduto as dbDeleteProduto, getProdutoById as dbGetProdutoById, getAllProdutosComPrecoVigente, updateEstoqueProduto, } from '../services/inventory.service.js';
function isStockDomainError(e) {
    if (!e || typeof e !== 'object')
        return false;
    const candidate = e;
    return typeof candidate.code === 'string' || typeof candidate.message === 'string';
}
const preprocessEstoque = z.preprocess((v) => (v === '' || v == null ? 0 : typeof v === 'string' ? Number(v) : v), z.number().int().min(0).refine((n) => !Number.isNaN(n), { message: 'Estoque inválido' }));
...
export async function getProdutos(_req) {
    try {
...
export async function getProdutoById(request) {
    try {
...
export async function updateProduto(request) {
    try {
        const tenantId = requireTenantFromRequest(request);
        const { id } = request.params;
        const data = updateSchema.parse(request.body);
        // Remove campos que não são colunas diretas (evita update quebrar)
        const { cores: _cores, variacoes: _variacoes, ...rest } = data;
        const patch = { ...rest };
        if (data.custo !== undefined)
            patch.custo = Number(data.custo).toFixed(2);
        if (data.valorVenda !== undefined)
            patch.valorVenda = Number(data.valorVenda).toFixed(2);
...
export async function deleteProduto(request) {
    try {
        const tenantId = requireTenantFromRequest(request);
        const { id } = request.params;
...
export async function buscarProdutos(request) {
    try {
        const tenantId = requireTenantFromRequest(request);
...
export async function atualizarEstoque(request) {
    try {
        const tenantId = requireTenantFromRequest(request);
        const { id } = request.params;
        const body = z.object({
            quantidade: z.preprocess((v) => (v === '' || v == null ? 0 : typeof v === 'string' ? Number(v) : v), z.number().int().min(0).refine((n) => !Number.isNaN(n), { message: 'Quantidade inválida' })),
            tipo: z.enum(['entrada', 'saida']),
        }).parse(request.body);
        const qtd = body.tipo === 'entrada' ? body.quantidade : -body.quantidade;
        await updateEstoqueProduto(tenantId, { id: Number(id), quantidade: qtd });
        return { message: 'Estoque atualizado' };
    }
    catch (e) {
        if (e instanceof z.ZodError)
            throw new TRPCError({ code: 'BAD_REQUEST', message: 'Dados inválidos' });
        if (isStockDomainError(e) && (e.code === 'ESTOQUE_NEGATIVO' || e.code === 'ESTOQUE_INSUFICIENTE')) {
...
export async function verificarEstoqueBaixo(_req) {
    try {
        const tenantId = requireTenantFromRequest(_req);
        // Mantido por compatibilidade (pode evoluir depois).
        const produtos = await getAllProdutosComPrecoVigente(tenantId, new Date());
        const baixo = produtos.filter((p) => Number(p.estoque ?? 0) <= 0);
