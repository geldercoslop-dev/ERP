...
function verificarEstoqueDisponivel(itens) {
...
function subtrairEstoque(itens) {
    for (const item of itens) {
        const produto = produtos.find(p => p.id === item.produto_id);
        if (produto) {
...
export async function getPedidos(request) {
    try {
...
export async function getPedidoById(request) {
    try {
...
export async function createPedido(request) {
    try {
...
export async function updatePedido(request) {
    try {
        const { id } = request.params;
        const pedidoData = pedidoSchema.parse(request.body);
...
export async function updateStatusPedido(request) {
    try {
        const { id } = request.params;
        const { status, data_entrega } = request.body;
        const index = pedidos.findIndex(p => p.id === parseInt(id));
        if (index === -1) {
            throw new TRPCError({ code: 'NOT_FOUND', message: 'Pedido não encontrado' });
        }
...
export async function deletePedido(request) {
    try {
        const { id } = request.params;
        const index = pedidos.findIndex(p => p.id === parseInt(id));
        if (index === -1) {
...
export async function buscarPedidos(request) {
    try {
        const { query, status, cliente_id, data_inicio, data_fim } = request.query;
        ...
        if (query) {
            ...
            pedidosFiltrados = pedidosFiltrados.filter(pedido => pedido.cliente?.nome.toLowerCase().includes(queryLower) ||
                String(pedido.id ?? "").includes(query));
        }
        // Filtro por status
        if (status) {
            pedidosFiltrados = pedidosFiltrados.filter(pedido => pedido.status === status.toUpperCase());
        }
        // Filtro por cliente
        if (cliente_id) {
            pedidosFiltrados = pedidosFiltrados.filter(pedido => pedido.cliente_id === parseInt(cliente_id));
        }
        // Filtro por período
        if (data_inicio && data_fim) {
            pedidosFiltrados = pedidosFiltrados.filter(pedido => pedido.data_pedido >= data_inicio && pedido.data_pedido <= data_fim);
        }
...
export async function getPedidosPorCliente(request) {
    try {
...
export async function getRelatorioVendas(request) {
    try {
        const { data_inicio, data_fim } = request.query;
        let pedidosRelatorio = pedidos;
        if (data_inicio && data_fim) {
            pedidosRelatorio = pedidos.filter(pedido => pedido.data_pedido >= data_inicio && pedido.data_pedido <= data_fim);
        }
        const relatorio = {
