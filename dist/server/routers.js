...
import { requireRole } from "./leo/runtime/trpc-auth.js";
...
import { checkRequestIdMemory, registerSuccessfulCreation, logDuplicationAttempt } from "./concurrency/pedido-control.js";
...
import { isDuplicateKeyError, createPedidoSafe, listPedidosTrpcPage, getPedidoWithItensForActor, getPedidoByIdForActor, getItensPedido, PedidoAccessError, } from "./services/orders.service.js";
import * as logisticaService from "./services/logistica.service.js";
import { buscarRegistros } from "./services/audit-service.js";
function toExpressRequest(value) {
    return value;
}
function isBcryptModuleLike(value) {
    if (!value || typeof value !== "object")
        return false;
    const candidate = value;
    return typeof candidate.hash === "function" && typeof candidate.compare === "function";
}
function hasStockErrorCode(error) {
...
async function getBcrypt() {
    ...
    try {
        ...
        const candidate = bcryptImported.default ?? bcryptImported;
        ...
        try {
            ...
            const bcryptHash = async (data, saltOrRounds) => {
                if (typeof saltOrRounds === 'string') {
                    ...
                    return await bcryptModule.hash(data, Math.abs(saltOrRounds));
                }
            };
            bcryptCache = {
                hash: bcryptHash,
                ...
                migratePlaintext: async (plaintext) => {
...
function mapPedidoAccessError(e) {
...
export const appRouter = router({
    ...
    auth: router({
            ...
            return runWithServiceInvocationAsync(buildBootstrapInvocation(1), async () => {
                ...
                if (user.role === "admin" || user.openId === "admin") {
                    ...
                    if (process.env.NODE_ENV !== "production")
                        console.log(`[auth.login] Cookie definido (admin)`);
                    return {
                        ok: true,
        ...
        impersonateVendedor: adminProcedure
            ...
            const currentToken = typeof ctx.req.headers.cookie === "string"
                ? cookie.parse(ctx.req.headers.cookie)[COOKIE_NAME]
                : undefined;
            if (currentToken) {
            ...
            if (!tenantId)
                throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Tenant inválido para impersonation" });
        ...
        stopImpersonation: protectedProcedure.mutation(async ({ ctx }) => {
            if (!ctx.isImpersonating)
                throw new TRPCError({ code: "BAD_REQUEST", message: "Não está em modo impersonation." });
            const rawCookie = ctx.req.headers.cookie;
            const parsed = rawCookie ? cookie.parse(rawCookie) : {};
            const adminToken = parsed[ADMIN_SESSION_COOKIE];
            if (!adminToken || typeof adminToken !== "string")
                throw new TRPCError({ code: "BAD_REQUEST", message: "Sessão admin não encontrada." });
            const cookieOptions = { ...getSessionCookieOptions(ctx.req), maxAge: ONE_YEAR_MS };
    ...
    vendedores: router({
        ...
        linkUser: adminProcedure
            ...
            if (!user)
                throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Falha ao criar/buscar usuário" });
    ...
    ajusteEstoque: router({
        rapido: adminProcedure
            ...
            const traceId = nanoid(10);
            const tenantId = await requireTenant(ctx);
            const out = await db.ajusteRapidoEstoque(tenantId, input.produtoId, input.quantidade, input.tipo, {
                actorUserId: ctx.user.id,
                traceId,
    ...
    clientes: router({
        ...
        search: protectedProcedure
            ...
            const tenantId = await requireTenant(ctx);
            if (ctx.user.role === "admin")
                return await db.searchClientes(tenantId, input.term);
            const vendedor = await getVendedorFromContext(ctx);
            if (!vendedor)
                return [];
        ...
        vinculate: protectedProcedure
            ...
            const vendedor = await getVendedorFromContext(ctx);
            if (!vendedor)
                throw new TRPCError({ code: "BAD_REQUEST", message: "Vendedor não identificado." });
            const tenantId = await requireTenant(ctx);
    ...
    pedidos: router({
        ...
        create: protectedProcedure
            ...
            const tenantId = await requireTenant(ctx);
            const actor = await resolveServiceActor(ctx);
            const trustedVendedorId = ctx.user.role === "admin" ? input.vendedorIdAlvo ?? ctx.vendedor?.id : undefined;
        ...
        update: protectedProcedure
            ...
            if (patch.observacoes !== undefined)
                data.observacoes = patch.observacoes;
            if (patch.formaPagamento !== undefined)
                data.formaPagamento = patch.formaPagamento;
        ...
        buscar: protectedProcedure
            ...
            const tenantId = await requireTenant(ctx);
            const filters = [db.eq(db.pedidos.tenantId, tenantId)];
            if (ctx.user.role !== "admin") {
                let ownerUid;
                try {
                ...
                const clienteVendedorSubquery = db_conn.select({ clienteId: db.clienteVendedores.clienteId })
                    ...
                    .where(db.and(db.eq(db.clienteVendedores.vendedorId, ownerUid)));
                filters.push(db.inArray(db.pedidos.clienteId, clienteVendedorSubquery));
            }
            if (input.query?.trim()) {
                const term = `%${input.query.trim()}%`;
                const buscaOr = db.or(db.sql `LOWER(${db.pedidos.clienteNome}) LIKE LOWER(${term})`, db.sql `${db.pedidos.numero} LIKE ${term}`);
                if (buscaOr)
                    filters.push(buscaOr);
            }
            ...
            const rows = await (whereSql === undefined ? base : base.where(whereSql)).orderBy(db.desc(db.pedidos.createdAt));
        ...
        listParaCarga: protectedProcedure.query(async ({ ctx }) => {
            ...
            const page = await logisticaService.getPedidosParaCarga(tenantId, filtrosCarga, { page: 1, pageSize: 200 });
            return {
                success: true,
                data: page.items,
                items: page.items,
        ...
        marcarConferido: protectedProcedure
            ...
            const result = await executeCommand({ commandName: "pedidos.marcarConferido", idempotencyKey: input.idempotencyKey }, async (tx) => {
        ...
        updateStatus: protectedProcedure
            ...
            const result = await executeCommand({ commandName: "pedidos.updateStatus", idempotencyKey: input.idempotencyKey ?? undefined }, async (tx) => {
        ...
        marcarEntregue: protectedProcedure
            ...
            try {
                const actor = await resolveServiceActor(ctx);
                const result = await executeCommand({ commandName: "baixarPedidoDireto", idempotencyKey: input.idempotencyKey }, async (tx) => {
                    const tenantId = await requireTenant(ctx);
                    const out = await db.baixarPedidoDireto(tenantId, input.id, {
        ...
        createVenda: protectedProcedure
            ...
            itens: z.array(z.object({
                ...
                produtoId: z.number().optional().nullable()
                    .refine((val) => val === null || val === undefined || val > 0, "ID do produto deve ser um número positivo"),
                corId: z.number().optional().nullable()
                    .refine((val) => val === null || val === undefined || val > 0, "ID da cor deve ser um número positivo"),
                corNome: z.string().optional().nullable(),
                ...
                .refine((items) => items.every(item => Number((item.quantidade * item.valorUnitario).toFixed(2)) ===
                Number((item.quantidade * item.valorUnitario).toFixed(2))), "Valores dos itens devem ter no máximo 2 casas decimais"),
            idempotencyKey: z.string().max(64).optional(),
        }))
            .mutation(async ({ input, ctx }) => {
            const createVendaStarted = Date.now();
            const perfPhases = process.env.CREATE_VENDA_PERF_LOG === "1" ? {} : undefined;
            const markPhase = (key, since) => {
                if (perfPhases)
                    perfPhases[key] = Date.now() - since;
            };
            try {
                ...
                const result = await executeCommand({ commandName: "createVenda", idempotencyKey }, async (tx) => {
                    let tMark = Date.now();
                    const isAdmin = ctx.user?.role === "admin";
                    let vendedor = ctx.vendedor ?? null;
                    ...
                    if (!clienteId) {
                        ...
                        const existing = await tx.select({ id: db.clientes.id }).from(db.clientes)
                            .where(db.and(db.eq(db.clientes.tenantId, vendedor.tenantId), db.eq(db.clientes.telefoneNorm, telefoneNorm), db.eq(db.clientes.nomeNorm, nn), db.eq(db.clientes.sobrenomeNorm, sn)))
                            .limit(1);
            ...
            `));
                    ...
                    const catalogIds = Array.from(new Set(input.itens
                        .filter((x) => x.tipo === 'CATALOGO' && typeof x.produtoId === 'number')
                        .map((x) => x.produtoId)));
                    ...
                    if (catalogIds.length > 0) {
                        const rows = await tx
                            ...
                            .from(db.produtos)
                            .where(db.and(db.eq(db.produtos.tenantId, vendedor.tenantId), db.inArray(db.produtos.id, catalogIds)))
                            .for("update");
                        for (const r of rows) {
                    ...
                    if (!pedidoId)
                        throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: 'Falha ao criar pedido.' });
                        ...
                        if (i.tipo === 'CATALOGO' && i.produtoId) {
                            ...
                            if (statusPedido === 'GERADO' && !falta) {
                                ...
                                await tx.update(db.produtos)
                                    ...
                                    .where(db.and(db.eq(db.produtos.tenantId, vendedor.tenantId), db.eq(db.produtos.id, i.produtoId)));
                                await db.insertAuditLog({
                                    actorUserId: ctx.user?.role === "admin" ? ctx.user.id : null,
                    ...
                    await tx.insert(db.itensPedido).values(itensPedidoRows);
    ...
    cargas: router({
        ...
        getPontosMapa: adminProcedure
            ...
            if (!payload)
                return { cidade: "", dataEntrega: "", pontos: [] };
            return payload;
        }),
    ...
    pendencias: router({
        list: protectedProcedure.query(async ({ ctx }) => {
            ...
            if (ctx.user?.role !== "admin" && vendedorId == null)
                return [];
    ...
    boletos: router({
        list: protectedProcedure
            ...
            if (input?.busca) {
                ...
                return boletosData.filter((b) => b.clienteNome?.toLowerCase().includes(termo) ||
        ...
        gerarExtrato: protectedProcedure
            ...
            if (ctx.user.role !== "admin") {
                ...
                if (doCliente.length === 0)
                    throw new TRPCError({ code: "FORBIDDEN", message: "Nenhum boleto seu para este cliente." });
        ...
        gerarBoletosCarga: protectedProcedure
            ...
            if (ctx.user.role !== "admin") {
                ...
                if (!carga)
                    throw new TRPCError({ code: "NOT_FOUND", message: "Carga não encontrada." });
                const pedidosIds = (carga.pedidos ?? []).map((p) => p.id);
                if (pedidosIds.length === 0)
                    throw new TRPCError({ code: "FORBIDDEN", message: "Carga sem pedidos." });
                const vendedor = await getVendedorFromContext(ctx);
                if (!vendedor)
                    throw new TRPCError({ code: "FORBIDDEN", message: "Acesso negado." });
                const db_conn = await db.getDb();
                ...
                const todosDoVendedor = rows.every((r) => r.vendedorId === vendedor.id);
                if (!todosDoVendedor)
                    throw new TRPCError({ code: "FORBIDDEN", message: "Carga contém pedidos de outro vendedor." });
            }
        ...
        gerarZip: protectedProcedure
            ...
            if (ctx.user.role !== "admin") {
                ...
                for (const bid of input.boletoIds) {
                    ...
                    if (!b)
                        throw new TRPCError({ code: "NOT_FOUND", message: `Boleto ${bid} não encontrado.` });
                    if (b.vendedorId !== vendedor.id)
                        throw new TRPCError({ code: "FORBIDDEN", message: "Acesso negado a um ou mais boletos." });
                }
            }
    ...
    contasReceber: router({
        ...
        create: protectedProcedure
            ...
            .mutation(async ({ input, ctx }) => {
            try {
                const result = await executeCommand({ commandName: "contasReceber.create", idempotencyKey: input.idempotencyKey }, async (tx) => {
                    let vendedorId;
                    if (ctx.user.role !== "admin") {
                        const vendedor = await getVendedorFromContext(ctx);
                        vendedorId = vendedor?.id;
    ...
    planoContas: router({
        ...
        update: adminProcedure
            ...
            .mutation(async ({ input }) => {
            const { idempotencyKey, ...data } = input;
            const result = await executeCommand({ commandName: "planoContas.update", idempotencyKey: idempotencyKey ?? undefined }, async (tx) => {
        ...
        delete: adminProcedure
            .input(z.object({ id: z.number(), idempotencyKey: z.string().max(64).optional() }))
            .mutation(async ({ input }) => {
            const result = await executeCommand({ commandName: "planoContas.delete", idempotencyKey: input.idempotencyKey ?? undefined }, async (tx) => {
    ...
    contasPagar: router({
        ...
        create: adminProcedure
            ...
            valor: z.string(),
            dataVencimento: z.preprocess((v) => (typeof v === "string" || typeof v === "number") ? new Date(v) : v, z.date().refine((d) => !Number.isNaN(d.getTime()), { message: "Data de vencimento inválida" })),
    ...
    audit: router({
        list: adminProcedure
            ...
            .query(async ({ input, ctx }) => {
            const tenantId = await requireTenant(ctx);
            const entidadeId = input?.pedidoId != null
    ...
    pedidoCompra: router({
        listPorFornecedor: protectedProcedure.query(async ({ ctx }) => {
            ...
            const pendRows = await dbConn
                ...
                .where(db.and(db.eq(db.pendencias.tenantId, tenantId), db.eq(db.pendencias.status, "PENDENTE"), db.eq(db.produtos.tenantId, tenantId), ...(vendedorId != null ? [db.eq(db.pendencias.vendedorId, vendedorId)] : [])));
            const map = new Map();
    ...
    dashboard: router({
        insights: protectedProcedure.query(async (opts) => {
            ...
            setDashboardCache(tenantId, data);
            await db.insertAuditLog({
                tenantId,
