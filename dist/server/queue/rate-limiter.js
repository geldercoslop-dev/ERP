...
class JobRateLimiter {
    static instance;
    debounceCache = new Map(); // local debounce timeouts
    config = new Map(); // jobType -> config
    constructor() { }
    static getInstance() {
     ...
     */
    async canExecute(jobType, entity, entityId) {
        ...
        try {
            ...
            if (count >= config.maxJobsPerMinute) {
                logger.warn(`Rate limit reached for ${jobType}: ${count}/${config.maxJobsPerMinute} jobs/min`);
                return false;
            }
     ...
     */
    debounce(jobType, entity, entityId, payload, callback) {
        const config = this.config.get(jobType);
     ...
     */
    async executeWithLimits(jobType, entity, entityId, payload, callback) {
     ...
     */
    generateDebounceKey(jobType, entity, entityId, payload) {
        const parts = [jobType];
        if (entity)
            parts.push(entity);
        if (entityId)
    ...
    async getStats() {
        ...
        try {
            ...
            for (const [jobType, config] of Array.from(this.config.entries())) {
                ...
                // Contar jobs
                const count = await redis.zcard(redisKey);
                const activeDebounces = Array.from(this.debounceCache.keys()).filter((k) => k.startsWith(jobType)).length;
                stats[jobType] = {
                    jobsLastMinute: count,
                    maxJobsPerMinute: config.maxJobsPerMinute || "unlimited",
                    activeDebounces,
                    utilizationRate: config.maxJobsPerMinute && config.maxJobsPerMinute > 0
    ...
    async cleanup() {
        ...
        if (redis) {
            try {
                ...
                logger.debug(`Cleanup complete: ${this.config.size} rate limit types monitored`);
            }
            catch (error) {
...
export async function executeJobWithLimits(jobType, entity, entityId, payload, callback) {
