 ...
 */
export function generateJobIdempotencyKey(config) {
 ...
 */
export async function wasJobExecuted(jobId) {
    try {
 ...
 */
export async function markJobStarted(jobId, jobType, entity, entityId, traceId) {
    try {
        const db = await getDb();
        if (!db)
            return false;
 ...
 */
export async function markJobCompleted(jobId, result, executionTimeMs) {
    try {
        const db = await getDb();
        if (!db)
            return;
 ...
 */
export async function markJobFailed(jobId, error, executionTimeMs) {
    try {
        const db = await getDb();
        if (!db)
            return;
 ...
 */
export async function getJobExecution(jobId) {
    try {
        const db = await getDb();
        if (!db)
            return null;
 ...
 */
export async function cleanupJobExecutions() {
    try {
        const db = await getDb();
        if (!db)
            return;
