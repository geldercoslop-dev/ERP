...
function isDatabaseError(error) {
    ...
    const dbError = error;
    if (dbError.code === 'PROTOCOL_ERROR' ||
        dbError.code === 'ER_' ||
 ...
 */
function mapDatabaseErrorToStatus(error) {
    if (error.code?.includes('ER_ACCESS_DENIED') ||
        error.code?.includes('ER_DBACCESS_DENIED')) {
        return 401; // Unauthorized
    }
    if (error.code?.includes('ER_NO_SUCH_TABLE') ||
 ...
 */
export async function withDatabaseFallback(operation, operationName, fallbackValue) {
    try {
        ...
        if (isDatabaseError(error)) {
            const dbError = error;
            logger.error(`Database operation failed: ${operationName}`, dbError, {
                metadata: {
...
export function databaseFallbackMiddleware() {
    return (err, req, res, next) => {
        ...
        logger.error(`Database error in request: ${req.method} ${req.path}`, dbError, {
            metadata: {
                path: req.path,
                method: req.method,
                code: dbError.code,
...
export class DatabaseFallbackHandler {
    static wrap(operation, operationName, defaultValue) {
        return withDatabaseFallback(operation, operationName, defaultValue);
    }
    /**
     * Para operações que PRECISAM de um valor
     */
    static async wrapRequired(operation, operationName) {
        try {
            return await operation();
        }
        catch (error) {
            if (isDatabaseError(error)) {
                logger.error(`Required database operation failed: ${operationName}`, error, {
                    metadata: {
                        operation: operationName,
                    },
                });
     ...
     */
    static async wrapArray(operation, operationName) {
        try {
            return await operation();
        }
        catch (error) {
            if (isDatabaseError(error)) {
                logger.error(`Database array operation failed: ${operationName}`, error, {
                    metadata: {
                        operation: operationName,
                    },
                });
     ...
     */
    static async wrapBoolean(operation, operationName) {
        try {
            return await operation();
        }
        catch (error) {
            if (isDatabaseError(error)) {
                logger.error(`Database boolean operation failed: ${operationName}`, error, {
                    metadata: {
                        operation: operationName,
                    },
                });
