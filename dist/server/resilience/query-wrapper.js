...
export async function executeWithResilience(operation, options = {}) {
    ...
    const circuitBreaker = CircuitBreakerManager.createCircuitBreaker(serviceName, {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            const result = await circuitBreaker.fire(async () => {
                return Promise.race([
                    operation(),
                    new Promise((_, reject) => setTimeout(() => reject(new Error(`Query timeout (${timeoutMs}ms)`)), timeoutMs)),
                ]);
            });
    ...
    throw new InfrastructureError(`Query failed after ${maxRetries} attempts: ${lastError?.message ?? 'Unknown error'}`);
}
/**
 * Convenience: Executa query com wrapper padrão (para uso em services/controllers)
 */
export async function queryWithResilience(operation, serviceName = 'database-query') {
 ...
 */
export async function healthCheckWithResilience(operation, serviceName = 'db-health-check') {
    return executeWithResilience(operation, {
        serviceName,
 ...
 */
export function resetQueryBreakerStats(serviceName) {
    if (serviceName) {
