...
export class BackpressureController {
    ...
    activeRequests = 0;
    queuedRequests = [];
    constructor(maxConcurrent = 100, maxQueueSize = 500, requestTimeout = 30000) {
     ...
     */
    setLimits(maxConcurrent, maxQueueSize) {
        this.maxConcurrent = maxConcurrent;
        this.maxQueueSize = maxQueueSize;
...
export const backpressureControllers = {
    api: new BackpressureController(BACKPRESSURE_CONFIG.API.maxConcurrent, BACKPRESSURE_CONFIG.API.maxQueueSize, BACKPRESSURE_CONFIG.API.requestTimeout),
    database: new BackpressureController(BACKPRESSURE_CONFIG.DATABASE.maxConcurrent, BACKPRESSURE_CONFIG.DATABASE.maxQueueSize, BACKPRESSURE_CONFIG.DATABASE.requestTimeout),
    fileUpload: new BackpressureController(BACKPRESSURE_CONFIG.FILE_UPLOAD.maxConcurrent, BACKPRESSURE_CONFIG.FILE_UPLOAD.maxQueueSize, BACKPRESSURE_CONFIG.FILE_UPLOAD.requestTimeout),
    externalApi: new BackpressureController(BACKPRESSURE_CONFIG.EXTERNAL_API.maxConcurrent, BACKPRESSURE_CONFIG.EXTERNAL_API.maxQueueSize, BACKPRESSURE_CONFIG.EXTERNAL_API.requestTimeout),
    heavyOperations: new BackpressureController(BACKPRESSURE_CONFIG.HEAVY_OPERATIONS.maxConcurrent, BACKPRESSURE_CONFIG.HEAVY_OPERATIONS.maxQueueSize, BACKPRESSURE_CONFIG.HEAVY_OPERATIONS.requestTimeout),
};
/**
 ...
 */
export function getAllBackpressureStatus() {
    return {
 ...
 */
export function adjustBackpressureLimits(cpuUsage, memoryUsage) {
    ...
    if (cpuUsage > 80 || memoryUsage > 80) {
        ...
        Object.values(backpressureControllers).forEach(controller => {
            ...
            controller.setLimits(Math.floor(current.maxConcurrent * reductionFactor), Math.floor(current.maxQueueSize * reductionFactor));
        });
        ...
        Object.values(backpressureControllers).forEach(controller => {
            const current = controller.getStatus();
            controller.setLimits(Math.floor(current.maxConcurrent * increaseFactor), Math.floor(current.maxQueueSize * increaseFactor));
        });
