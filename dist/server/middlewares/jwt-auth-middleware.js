...
function resolveSessionLikeToken(req) {
    ...
    const authToken = typeof authHeader === 'string' ? authHeader.replace(/^\s*Bearer\s+/i, '').trim() : undefined;
...
function mapUserToJwtPayload(u) {
...
function mapVendedorToJwtPayload(v, tenantId) {
    return {
        userId: v.id,
        tenantId,
        email: v.email ?? '',
 ...
 */
async function resolveSessionTokenToPayload(token) {
    ...
    if (token.startsWith('v:')) {
        ...
        if (!Number.isInteger(tenantId) || tenantId <= 0 || !Number.isInteger(id) || id <= 0)
            return null;
...
export function requireTenantAccess(req, res, next) {
    ...
    const hasTenantOverride = typeof req.params.tenantId !== 'undefined' ||
