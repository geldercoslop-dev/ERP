...
const blockedIPs = new BoundedMap(5000);
/**
 ...
 */
function getRateLimitEntry(ip, config) {
    const now = Date.now();
...
function setRateLimitHeaders(res, config, entry) {
    ...
    if (res.req.protocol === 'https' || res.req.secure) {
 ...
 */
function logAbuse(ip, path, reason, details = {}) {
    logger.error({
        ip,
        path,
        reason,
 ...
 */
export function createRateLimitMiddleware(options = DEFAULT_CONFIG) {
