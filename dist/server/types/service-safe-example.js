 ...
 */
import { sanitizeList, safeListCall, } from './service-safety.js';
 ...
 */
export async function getAllProdutosComPrecoVigenteUNSAFE(refDate = new Date()) {
    const db = await getDb();
    if (!db)
        return []; // ❌ Tipo incoerente
    const items = await db.select().from(produtos).where(eq(produtos.ativo, true)).orderBy(produtos.nome);
    const ids = items.map((p) => p.id);
    if (ids.length === 0)
        return [];
    // ❌ PROBLEMA: execute() retorna [[rows], metadata]
    // ❌ O código faz row?.[0] que pode ser undefined
    const variacoesResult = await db.execute(sql `...`);
    ...
    return items.map((p) => ({ id: p.id, nome: p.nome })); // Pode retornar items com dados incompletos
}
/**
 * ============================================================
 ...
 * - Type-safe
 */
export async function getAllProdutosComPrecoVigenteV1(refDate = new Date()) {
    const db = await getDb();
    if (!db)
        return [];
    const items = await db.select().from(produtos).where(eq(produtos.ativo, true)).orderBy(produtos.nome);
 ...
 */
export async function getAllProdutosComPrecoVigenteV2(refDate = new Date()) {
    return await safeListCall(async () => {
        const db = await getDb();
        if (!db)
            return undefined;
 ...
 */
export async function getAllProdutosComPrecoVigenteV3(refDate = new Date()) {
 ...
 */
export async function getAllProdutosComPrecoVigenteRECOMMENDED(refDate = new Date()) {
    return await withServiceGuard(() => safeListCall(async () => {
        const db = await getDb();
        if (!db)
            return undefined;
        const items = await db
        ...
        return items.map((p) => {
            ...
            const vigente = promo?.precoPromo && promo.precoPromo > 0
                ? promo.precoPromo
                : base;
...
const getDb = async () => ({
    select: () => ({
        from: (table) => ({
            where: (condition) => ({
                orderBy: (order) => Promise.resolve([{ id: 1, nome: 'test', ativo: true, descricao: 'test', valorVenda: 100 }])
            })
        })
    })
});
...
const sql = (template, ...values) => template;
const eq = (a, b) => a;
const asc = (col) => col;
