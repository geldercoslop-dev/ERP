...
export class TracingValidator {
     ...
     */
    async testNestedSpans() {
        logger.info('Testing nested spans');
        try {
            // Criar span pai
            const parentSpan = tracer.startSpan('parent_operation');
            // Criar span filho
            const childSpan = tracer.startSpan('child_operation', tracer.getCurrentContext(parentSpan.spanId) || undefined);
    ...
    async runAllTests() {
        ...
        try {
            ...
            logger.error('Tracing validation test suite failed', error);
        }
    }
    /**
 ...
 */
export async function runTracingValidation() {
