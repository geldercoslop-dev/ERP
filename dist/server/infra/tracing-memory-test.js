...
export async function testMemoryLeakDetection() {
    ...
    try {
        ...
        for (let i = 0; i < operationCount; i++) {
            operations.push(withTracing(`test_operation_${i}`, async () => {
                // Simular operação assíncrona
                await new Promise(resolve => setTimeout(resolve, Math.random() * 10));
            }, { 'test.index': i }).catch(error => {
 ...
 */
export async function testContextIsolation() {
    logger.info('Starting context isolation test');
    const requestCount = 50;
    const contextMismatches = [];
    try {
        // Criar múltiplos requests simultâneos
        const requests = Array.from({ length: requestCount }, (_, index) => withTracing(`request_${index}`, async () => {
