...
export function requestTracingMiddleware() {
    return (req, res, next) => {
        ...
        const span = tracer.startSpan(`${req.method} ${req.path}`, undefined, {
...
export function databaseTracingMiddleware() {
    return (req, res, next) => {
        ...
        if (originalQuery) {
            req.app.locals.database.query = async function (sql, params) {
                return withTracing(`database.query`, async () => {
...
export function leoTracingMiddleware() {
    return (req, res, next) => {
        ...
        if (originalLeoCall) {
            req.app.locals.leo.execute = async function (prompt, options) {
                return withTracing('leo.ai.execution', async () => {
                    const startTime = Date.now();
                    try {
...
export function serviceTracingMiddleware() {
    return (req, res, next) => {
        ...
        if (originalServiceCall) {
            req.app.locals.services.call = async function (serviceName, method, ...args) {
                return withTracing(`service.${serviceName}.${method}`, async () => {
                    const startTime = Date.now();
                    try {
 ...
 */
export function createCustomSpan(req, operationName, tags) {
    const parentSpan = req.traceSpan;
    if (!parentSpan)
        return null;
    const span = tracer.startSpan(operationName, tracer.getCurrentContext(parentSpan.spanId) ?? undefined, {
 ...
 */
export function logWithTrace(req, level, message, metadata) {
    const traceMetadata = {
        traceId: req.traceId,
        ...metadata,
    };
