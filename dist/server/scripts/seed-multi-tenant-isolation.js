...
async function hashPassword(plain) {
    ...
    const bcrypt = bcryptImported.default ?? bcryptImported;
...
function uniqPhone(base, suffix) {
...
async function main() {
     ...
     */
    const [maxRows] = await connection.execute(`SELECT COALESCE(MAX(tenant_id), 0) AS maxTenantId FROM users`);
    const maxTenantId = Array.isArray(maxRows) ? maxRows[0]?.maxTenantId : 0;
    ...
    const userA = await db.insertUser({
        ...
        lastSignedIn: toDbDateStrict(new Date()),
    });
