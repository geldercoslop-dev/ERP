...
async function main() {
    ...
    `);
        const [idx] = await conn.execute(sql `
    ...
    `);
        ...
        rows.forEach((r) => console.log(r.COLUMN_NAME, r.COLUMN_TYPE, "DEFAULT", r.COLUMN_DEFAULT));
        console.log("\n--- Índices ---");
        (idx || []).forEach((r) => console.log(r.INDEX_NAME, "UNIQUE:", r.NON_UNIQUE === 0, "COL:", r.COLUMN_NAME));
        const uniqueCmdKey = (idx || []).find((r) => r.INDEX_NAME === "idempotency_cmd_key" && r.NON_UNIQUE === 0);
        if (!uniqueCmdKey) {
            console.error("\nFALHA: índice UNIQUE idempotency_cmd_key (commandName, key) não encontrado.");
            process.exit(1);
        }
        const createdAtIndex = (idx || []).find((r) => r.INDEX_NAME === "idempotency_created_at_idx" && r.COLUMN_NAME === "created_at");
        if (!createdAtIndex) {
            console.error("\nFALHA: índice idempotency_created_at_idx em created_at não encontrado.");
            process.exit(1);
        }
        const createdAtCol = (rows || []).find((r) => r.COLUMN_NAME === "created_at");
