...
async function main() {
    ...
    try {
        ...
        console.error("[health-check] Falha: consulta pedidos:", e?.message ?? e);
        process.exit(1);
    }
    try {
        ...
        console.error("[health-check] Falha: consulta produtos:", e?.message ?? e);
        process.exit(1);
    }
    try {
        ...
        console.error("[health-check] Falha: tabela sessões inacessível:", e?.message ?? e);
        process.exit(1);
    }
