...
router.post('/test-concurrency', async (req, res) => {
    try {
        ...
        const result = await testConcurrency(Number(produtoId) || 1, Number(quantidade) || 5, Number(testId) || Date.now());
...
router.post('/test-deadlock', async (req, res) => {
    try {
        ...
        const result = await testDeadlock(Number(produto1Id) || 1, Number(produto2Id) || 2, Number(testId) || Date.now());
        res.json(result);
    }
    catch (error) {
