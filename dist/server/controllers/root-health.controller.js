...
import { buildSystemHealthFailureResponse, getSystemHealthComplete, getSystemHealthLight, } from "../services/system-health.service.js";
...
router.get("/full", async (req, res) => {
    try {
        ...
        res.status(httpStatus).json({ ...healthData, checks: "full" });
    }
    catch (error) {
        systemLogger.error({
        ...
        res.status(503).json({ ...buildSystemHealthFailureResponse(error), checks: "full" });
    }
});
