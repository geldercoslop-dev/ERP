...
import { reserveStockForOrder } from '../services/stock-safety.service.js';
 ...
 */
export async function createOrderSafe(orderData) {
    return runTransaction(async (tx) => {
        ...
        const [produtosRows] = await tx.execute(`SELECT id, descricao, estoque, ativo FROM produtos WHERE tenantId = ? AND id IN (${placeholders}) FOR UPDATE`, [orderData.tenantId, ...produtoIds]);
        ...
        if (!pedidoNumero) {
            const [numeroResult] = await tx.execute(`SELECT COALESCE(MAX(numero), 0) + 1 as proximoNumero FROM pedidos WHERE tenantId = ?`, [orderData.tenantId]);
            pedidoNumero = numeroResult[0]?.proximoNumero || 1;
        }
        // 6. Inserir pedido
        const [pedidoResult] = await tx.execute(`INSERT INTO pedidos (
        ...
        for (const item of orderData.itens) {
            const [itemResult] = await tx.execute(`INSERT INTO itens_pedido (
          tenant_id, pedidoId, produtoId, quantidade, valorUnitario, descricao, tipo, custo, prazoGarantia, createdAt
        ) VALUES (?, ?, ?, ?, ?, ?, 'LIVRE', 0, 90, NOW())`, [orderData.tenantId, pedidoId, item.produtoId, item.quantidade, item.valorUnitario, item.descricao ?? '']);
            itensInseridos.push({
                ...item,
        ...
        const movimentacoesEstoque = await reserveStockForOrder(orderData.tenantId, orderData.itens.map(item => ({
            produtoId: item.produtoId,
            quantidade: item.quantidade
        })), pedidoId, orderData.usuarioId, orderData.vendedorId);
 ...
 */
export async function cancelOrderSafe(tenantId, // Adicionado para multi-tenant
pedidoId, motivo = 'Cancelamento', usuarioId, vendedorId) {
    if (!tenantId)
    ...
    return runTransaction(async (tx) => {
        ...
        const [pedido] = await tx.execute(`SELECT id, numero, status, clienteNome, total FROM pedidos WHERE tenantId = ? AND id = ? FOR UPDATE`, [tenantId, pedidoId]);
        if (!pedido || !pedido[0]) {
        ...
        const [itens] = await tx.execute(`SELECT produtoId, quantidade FROM itens_pedido WHERE pedidoId = ?`, [pedidoId]);
        const itensPedido = itens;
        // 4. Devolver estoque (usando reserveStockForOrder que agora aceita tenantId)
        const movimentacoesEstoque = await reserveStockForOrder(tenantId, itensPedido.map(item => ({
            produtoId: item.produtoId,
            quantidade: -item.quantidade // Devolução (quantidade negativa)
        })), pedidoId, usuarioId, vendedorId);
        // 5. Atualizar status do pedido com tenantId
        await tx.execute(`UPDATE pedidos SET status = ?, updatedAt = NOW() WHERE tenantId = ? AND id = ?`, ['CANCELADO', tenantId, pedidoId]);
 ...
 */
export async function updateOrderStatusSafe(tenantId, // Adicionado para multi-tenant
pedidoId, novoStatus, motivo, usuarioId, vendedorId) {
    if (!tenantId)
        throw new ValidationError("tenantId is required");
    return runTransaction(async (tx) => {
        ...
        const [pedido] = await tx.execute(`SELECT id, numero, status FROM pedidos WHERE tenantId = ? AND id = ? FOR UPDATE`, [tenantId, pedidoId]);
        if (!pedido || !pedido[0]) {
            throw new ValidationError('PEDIDO_NAO_ENCONTRADO: Pedido não encontrado ou acesso negado');
        }
        const pedidoData = pedido[0];
        ...
        await tx.execute(`UPDATE pedidos SET status = ?, updatedAt = NOW() WHERE tenantId = ? AND id = ?`, [novoStatus, tenantId, pedidoId]);
 ...
 */
export async function validateOrderIntegrity(tenantId, pedidoId) {
    if (!tenantId)
        throw new ValidationError("tenantId is required");
    ...
    try {
        ...
        const [pedidoRows] = await pool.execute(`SELECT * FROM pedidos WHERE tenantId = ? AND id = ?`, [tenantId, pedidoId]);
        ...
        // 2. Buscar itens
        const [itensRows] = await pool.execute(`SELECT * FROM itens_pedido WHERE pedidoId = ?`, [pedidoId]);
        const itensPedido = Array.isArray(itensRows) ? itensRows : [];
        // 3. Validar soma dos itens
        const somaItens = itensPedido.reduce((sum, item) => sum + Number(item.total ?? 0), 0);
        ...
        if (pedidoData.status !== 'CANCELADO') {
            for (const item of itensPedido) {
                const [produtoRows] = await pool.execute(`SELECT estoque FROM produtos WHERE id = ?`, [Number(item.produtoId)]);
