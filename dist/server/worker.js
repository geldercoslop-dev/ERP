...
async function startWorker() {
    try {
        ...
        systemLogger.error({ error: error instanceof Error ? error.message : String(error) }, "[WORKER] falha ao iniciar");
        process.exit(1);
    }
}
async function shutdownWorker(signal) {
    ...
    try {
        ...
        systemLogger.error({ error: error instanceof Error ? error.message : String(error) }, "[WORKER] falha ao encerrar");
        process.exit(1);
    }
}
process.on("SIGINT", () => {
