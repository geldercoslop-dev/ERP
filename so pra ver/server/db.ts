  ...
  try {
    ...
    await (db as any).execute(sql`
      CREATE TABLE IF NOT EXISTS audit_log (
        ...
        actorVendedorId INT NULL,
        action VARCHAR(32) NOT NULL,
        entity VARCHAR(64) NOT NULL,
        entityId VARCHAR(64) NULL,
        payloadJson TEXT NULL,
        traceId VARCHAR(32) NULL,
        INDEX audit_entity_idx (entity),
        INDEX audit_entity_id_idx (entityId),
        INDEX audit_created_at_idx (createdAt)
      ) ENGINE=InnoDB;
    `);

    // idempotency_keys: NÃO criar aqui. Fonte única = migration 0005 (drizzle/0005_idempotency_keys.sql).
    // Se a tabela não existir, rode: npm run db:migrate
  } catch (e) {
...
/** Clientes que possuem ao menos um pedido do vendedor (isolamento RBAC sem coluna vendedorId em clientes). */
export async function listClientesByVendedor(vendedorId: number) {
  const db = await getDb();
  if (!db) return [];
  const rows = await db.select({ clienteId: pedidos.clienteId }).from(pedidos).where(eq(pedidos.vendedorId, vendedorId));
...
/** Busca clientes por termo restrita a clientes com pedido do vendedor. */
...
/** Retorna true se o cliente tem ao menos um pedido do vendedor (ownership para update). */
...
export async function listContasPagarFiltro(status?: 'PENDENTE' | 'PAGO', fornecedor?: string) {
  ...
  // Schema atual não possui nome do fornecedor em contas_pagar (apenas fornecedorId).
  // Para manter a busca por texto, filtramos por descrição.
  if (fornecedor?.trim()) conditions.push(sql`${contasPagar.descricao} LIKE ${'%' + fornecedor.trim() + '%'}`);
  ...
  for (const f of fixas) {
    ...
    if (existe.length === 0) {
      await db.insert(contasPagar).values({
        ...
        dataVencimento,
        status: "PENDENTE",
        planoContaId: (f as any).planoContaId ?? null,
        fornecedorId: null,
      });
    }
  }
