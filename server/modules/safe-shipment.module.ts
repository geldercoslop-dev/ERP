  ...
  return runTransaction(async (tx: TransactionConnection) => {
    ...
    for (const item of shipmentData.itens) {
      ...
      const [pedido] = await tx.execute(
        `SELECT id, numero, status, cliente_nome, total, peso, volume FROM pedidos WHERE id = ? FOR UPDATE`,
        [item.pedidoId]
      );
      ...
      const [emCarga] = await tx.execute(
        `SELECT COUNT(*) as total FROM pedidos_carga WHERE pedido_id = ?`,
        [item.pedidoId]
      );
    ...
    // 4. Inserir carga
    const [shipmentResult] = await tx.execute(
      `INSERT INTO cargas (
...
export async function startShipmentSafe(
  ...
  return runTransaction(async (tx: TransactionConnection) => {
    ...
    const [shipment] = await tx.execute(
      `SELECT id, placa, motorista, status, tenant_id FROM cargas WHERE id = ? FOR UPDATE`,
      [shipmentId]
    );
    ...
    await tx.execute(
      `UPDATE cargas SET status = 'EM_TRANSITO', dataSaidaReal = NOW(), updated_at = NOW() WHERE id = ?`,
      [shipmentId]
    );

    // 4. Atualizar status dos pedidos na carga
    await tx.execute(
      `UPDATE pedidos SET status = 'ENTREGA', updated_at = NOW() 
       WHERE id IN (SELECT pedido_id FROM pedidos_carga WHERE carga_id = ?)`,
      [shipmentId]
    );
 ...
 */
export async function finishShipmentSafe(
  shipmentId: number,
  observacoes?: string,
  tenantId?: number,
  usuarioId?: number,
  ...
  return runTransaction(async (tx: TransactionConnection) => {
    ...
    // 3. Atualizar status da carga
    await tx.execute(
      `UPDATE cargas SET status = 'ENTREGUE', dataChegadaReal = NOW(), observacoesFinais = ?, updated_at = NOW() WHERE id = ?`,
      [observacoes || '', shipmentId]
    );

    // 4. Atualizar status dos pedidos na carga
    await tx.execute(
      `UPDATE pedidos SET status = 'ENTREGUE', data_entrega = NOW(), updated_at = NOW() 
       WHERE id IN (SELECT pedido_id FROM pedidos_carga WHERE carga_id = ?)`,
      [shipmentId]
    );
    ...
    await tx.execute(
      `UPDATE pedidos_carga SET status = 'ENTREGUE', data_entrega = NOW() WHERE carga_id = ?`,
      [shipmentId]
    );
 ...
 */
export async function removeOrderFromShipmentSafe(
  shipmentId: number,
  pedidoId: number,
  motivo: string = 'Remoção manual',
  tenantId?: number,
  usuarioId?: number,
  vendedorId?: number
): Promise<ShipmentResult> {
  return runTransaction(async (tx: TransactionConnection) => {
    ...
    // 1. Validar carga
    const [shipment] = await tx.execute(
      `SELECT id, status, tenant_id FROM cargas WHERE id = ? FOR UPDATE`,
      [shipmentId]
    );
    ...
    const [pedidoNaCarga] = await tx.execute(
      `SELECT COUNT(*) as total FROM pedidos_carga WHERE carga_id = ? AND pedido_id = ?`,
      [shipmentId, pedidoId]
    );
    ...
    await tx.execute(
      `DELETE FROM pedidos_carga WHERE carga_id = ? AND pedido_id = ?`,
      [shipmentId, pedidoId]
    );
 ...
 */
export async function getShipmentsReport(
  filtros: {
    status?: string;
    placa?: string;
    motorista?: string;
 ...
 */
export async function validateShipmentsIntegrity(): Promise<{
  valido: boolean;
  erros: string[];
