 ...
 */
async function geocodeEndereco(endereco: string): Promise<{ lat: number; lng: number }> {
  try {
    ...
    if (arr.length > 0) {
      const result = arr[0] as { lat: string; lon: string };
      return {
...
export async function obterPontosMapaCarga(tenantId: number, cargaId: number): Promise<{ cidade: string; dataEntrega: string; pontos: PontoMapa[] } | null> {
  ...
  const geocodePromises = pedidos.map(async (p: unknown) => {
