...
export interface PontoRota {
  ...
  bairro: string;
  cidade: string;
  endereco?: string; // endereço completo para geocoding
}

export interface RotaSugeridaResult {
  ordem: number[];
  detalhes?: { pedidoCargaId: number; ordemEntrega: number }[];
  via: "openroute" | "fallback";
}
 ...
 */
export async function gerarRotaSugerida(pontos: PontoRota[]): Promise<RotaSugeridaResult> {
  if (!pontos.length) return { ordem: [], detalhes: [], via: "fallback" };
  ...
  if (OPENROUTE_API_KEY && pontos.length >= 2) {
    try {
      ...
      console.warn("[rota-sugerida] OpenRoute falhou, usando fallback:", (e as Error).message);
    }
  }
...
async function geocodePontos(pontos: PontoRota[]): Promise<{ lat: number; lng: number; pedidoCargaId: number }[]> {
  const out: { lat: number; lng: number; pedidoCargaId: number }[] = [];
...
function ordenarPorProximidade(coords: { lat: number; lng: number; pedidoCargaId: number }[]): number[] {
  if (coords.length <= 1) return coords.map((_, i) => i);
