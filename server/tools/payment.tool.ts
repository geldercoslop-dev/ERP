...
export class PaymentTool {
   ...
   */
  async create(input: Payload): Promise<Record<string, unknown>> {
   ...
   */
  async list(input: Payload): Promise<Record<string, unknown>> {
    console.log(`[PaymentTool] Listing payments:`, input);
    
    // Normalize and validate input
    const normalized = {
      page: Math.max(1, (input.page as number) || 1),
      limit: Math.min(100, Math.max(1, (input.limit as number) || 50)),
   ...
   */
  async update(input: Payload): Promise<Record<string, unknown>> {
