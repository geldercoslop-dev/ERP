 ...
 */
export function assertTenantSource(
  tenantId: string | number | undefined,
  source: 'request' | 'param' | 'body'
): string | number {
 ...
 */
export function extractTenantFromRequest(req: unknown): string | number {
  // Try multiple sources in order of preference
  const r = req as { tenantId?: unknown; user?: { tenantId?: unknown }; params?: { tenantId?: unknown }; query?: { tenantId?: unknown } } | undefined;
  const tenantId = 
    r?.tenantId ||
    r?.user?.tenantId ||
    r?.params?.tenantId ||
  ...
  return assertTenant(tenantId as string | number | undefined, 'request');
}
...
export function isTenantValid(
  tenantId: string | number | undefined
): tenantId is Exclude<string | number, 0 | ''> {
