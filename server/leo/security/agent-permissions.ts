...
export interface PermissionContext {
  tenantId: number;
  userRole?: string;
  /** Papel canônico (preferir em novos fluxos). */
  role?: SecureRole;
  userId?: number;
  vendedorId?: number;
  action: string;
...
export class AgentPermissions {
  private static instance: AgentPermissions;
  ...
  static getInstance(): AgentPermissions {
   ...
   */
  validateRequiredContext(context: PermissionContext): {
    valid: boolean;
    reason?: string;
  } {
    if (!context.tenantId) {
  ...
  private initializeRules() {
    ...
    const erpRead = ['admin', 'user', 'vendedor'] as const;
   ...
   */
  hasPermission(toolName: string, context: PermissionContext): {
    allowed: boolean;
    reason?: string;
    requiresConfirmation: boolean;
    dangerous: boolean;
   ...
   */
  private validateSafeExecution(toolName: string, context: PermissionContext): boolean {
  ...
  sanitizeFilePath(filePath: string): string {
    ...
    const isRestricted = restrictedPaths.some(restricted => 
      cleanPath.toLowerCase().startsWith(restricted.toLowerCase())
    );
   ...
   */
  getRulesByCategory(category: 'desktop' | 'system' | 'erp'): PermissionRule[] {
