...
export interface WatchdogConfig {
  ...
  maxDowntime: number; // milissegundos antes de reiniciar
  maxRestartAttempts: number;
  restartCooldown: number; // milissegundos entre tentativas de reinício
...
export interface WatchdogStatus {
  ...
  lastCheck: Date;
  lastLoopActivity: Date;
  restartAttempts: number;
  lastRestart: Date;
  downtimeCount: number;
  totalDowntime: number;
  status: 'healthy' | 'warning' | 'critical' | 'restarting';
}
 ...
 */
class LeoWatchdog {
  private static instance: LeoWatchdog;
  ...
  private lastLoopStatus?: { running: boolean; timestamp: Date };

  private constructor() {
    this.initialize();
  }

  public static getInstance(): LeoWatchdog {
   ...
   */
  private async checkLoopHealth(): Promise<void> {
    try {
      this.status.lastCheck = new Date();
      ...
      if (!currentStatus.running) {
        const lastActivity = (currentStatus as { lastDecisionTime?: number; timestamp?: Date }).lastDecisionTime ?? (currentStatus as { timestamp?: Date }).timestamp?.getTime() ?? Date.now();
   ...
   */
  private async handleLoopStopped(downtime: number): Promise<void> {
    try {
   ...
   */
  private async handleLongInactivity(inactivityTime: number): Promise<void> {
    try {
      this.status.status = 'critical';
   ...
   */
  private async restartLoop(): Promise<{ success: boolean; error?: string }> {
    try {
