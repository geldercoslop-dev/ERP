...
export const createError = {
  validation: (message: string, details?: Record<string, unknown>, requestId?: string) =>
    new ERPError(ErrorCode.VALIDATION_ERROR, message, 400, details, requestId),
  
  unauthorized: (message: string = 'Não autorizado', requestId?: string) =>
    new ERPError(ErrorCode.UNAUTHORIZED, message, 401, undefined, requestId),
  
  forbidden: (message: string = 'Acesso negado', requestId?: string) =>
    new ERPError(ErrorCode.FORBIDDEN, message, 403, undefined, requestId),
  
  notFound: (resource: string, id?: unknown, requestId?: string) =>
    new ERPError(ErrorCode.RECORD_NOT_FOUND, `${resource}${id ? ` com ID ${id}` : ''} não encontrado`, 404, { resource, id }, requestId),
  
  database: (message: string, details?: Record<string, unknown>, requestId?: string) =>
    new ERPError(ErrorCode.DATABASE_ERROR, message, 500, details, requestId),
  
  business: (message: string, details?: Record<string, unknown>, requestId?: string) =>
    new ERPError(ErrorCode.BUSINESS_RULE_VIOLATION, message, 422, details, requestId),
  
  stock: (message: string, productId?: number, requestId?: string) =>
    new ERPError(ErrorCode.INSUFFICIENT_STOCK, message, 422, { productId }, requestId),
  
  external: (service: string, message: string, details?: Record<string, unknown>, requestId?: string) =>
    new ERPError(ErrorCode.EXTERNAL_API_ERROR, `Erro no serviço ${service}: ${message}`, 502, details, requestId),
  
  timeout: (operation: string, timeout: number, requestId?: string) =>
    new ERPError(ErrorCode.TIMEOUT_ERROR, `Timeout na operação ${operation} (${timeout}ms)`, 408, { operation, timeout }, requestId),
  
  rateLimit: (limit: number, window: string, requestId?: string) =>
    new ERPError(ErrorCode.RATE_LIMIT_EXCEEDED, `Limite de requisições excedido: ${limit} por ${window}`, 429, { limit, window }, requestId)
};
...
export function withErrorHandling<TInput, TOutput>(
  procedure: {
    use: (middleware: (opts: { input: TInput; ctx: Record<string, unknown>; next: Function }) => Promise<TOutput>) => unknown;
  },
  handler: (opts: { input: TInput; ctx: Record<string, unknown> }) => Promise<TOutput>
): unknown {
 ...
 */
export function debugError(error: Error): {
  isERPError: boolean;
  isTRPCError: boolean;
  errorInfo: {
    name: string;
  ...
  if (isERPError) {
    suggestedAction = `Verificar o código de erro: ${(error as ERPError).code}`;
  } else if (isTRPCError) {
    suggestedAction = `Verificar o código tRPC: ${(error as TRPCError).code}`;
