...
export function getProjectRoot(): string {
  ...
  if (envRoot && path.isAbsolute(envRoot)) {
    ...
    console.error(
      `[PROJECT_ROOT] PROJECT_ROOT ignorado (sem package.json em ${envRoot}). Usando detecção por import.meta.url.`
    );
  }
  ...
  if (!fs.existsSync(marker)) {
    console.error(
      `[PROJECT_ROOT] Raiz inferida sem package.json: ${root}. Defina PROJECT_ROOT ou mantenha o layout dist/server/_core ou server/_core.`
    );
  }
  return root;
}
