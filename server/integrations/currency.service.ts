 ...
 */
export async function cotacaoMoeda(
  moeda: "USD" | "EUR" | "BTC" = "USD"
): Promise<{ ok: boolean; cotacao?: CotacaoMoeda; erro?: string }> {
  ...
  return getOrSet(`currency:${par}`, async () => {
    try {
      ...
      const item = data[key];
      if (!item) return { ok: false, erro: "Resposta inválida." };
      return {
        ok: true,
        cotacao: {
