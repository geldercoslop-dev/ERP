...
export type SerieVendas = { label: string; valor: number }[];

/**
 * Gera URL de gráfico de barras de vendas (últimos dias ou por período).
 * O front pode exibir: <img src={url} alt="Gráfico vendas" />
 */
export function gerarGraficoVendasUrl(series: SerieVendas, titulo = "Vendas"): string {
 ...
 */
export function gerarGraficoLinhaUrl(
  labels: string[],
  valores: number[],
  titulo = "Vendas"
): string {
