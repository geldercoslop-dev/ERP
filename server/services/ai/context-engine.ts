/**
 * Motor de contexto do LEO: data atual, dia da semana, dias restantes do ano.
 */
...
export function getContextoData(): ContextoData {
  const hoje = new Date();
...
export function responderContexto(pergunta: string): string | null {
  ...
  if (/dia\s+da\s+semana|qual\s+dia/.test(p)) return `Hoje é ${ctx.diaDaSemana}.`;
