/**
 * Motor universal de perguntas: interpreta perguntas em linguagem natural.
 * Processo: normalizar texto → remover acentos → identificar palavras-chave → extrair entidades.
 */
export type EntidadesExtraidas = {
  numeroPedido?: number;
  nomeCliente?: string;
  nomeProduto?: string;
  data?: string;
  periodo?: "hoje" | "ontem" | "semana" | "mes";
  tipoRelatorio?: "estoque" | "vendas" | "clientes" | "produtos";
...
export function extrairNumeroPedido(texto: string): number | null {
  ...
  const match =
    /(?:pedido|numero|num|n[º°]?|#)\s*(\d+)/i.exec(n) ||
    /(\d{3,})\s*(?:pedido)?/i.exec(n);
  if (match && match[1]) return parseInt(match[1], 10);
  const onlyNum = /^(\d+)$/.exec(n);
  if (onlyNum && onlyNum[1]) return parseInt(onlyNum[1], 10);
  const pedidoNum = /pedido\s+(\d+)/i.exec(texto);
  if (pedidoNum && pedidoNum[1]) return parseInt(pedidoNum[1], 10);
  return null;
}
...
export function extrairNomeCliente(texto: string): string | null {
  ...
  if (withQuotes && withQuotes[1]) return withQuotes[1].trim();
  const afterCliente = /(?:pedidos?\s+do\s+cliente|cliente)\s+([a-z0-9\s]+?)(?:\s*\?|$)/i.exec(n);
  if (afterCliente && afterCliente[1]) return afterCliente[1].trim().slice(0, 120);
  return null;
}

/** Extrai nome de produto: após "produto", "estoque de", "produto ...". */
export function extrairNomeProduto(texto: string): string | undefined {
  const n = normalizarTexto(texto);
  ...
  if (afterProduto && afterProduto[1]) return afterProduto[1].trim().slice(0, 120);
  return undefined;
}

/** Extrai período: hoje, ontem, semana, mês. */
export function extrairPeriodo(texto: string): EntidadesExtraidas["periodo"] {
  const n = normalizarTexto(texto);
  if (/\bhoje\b/.test(n)) return "hoje";
  if (/\bontem\b/.test(n)) return "ontem";
  if (/\bsemana\b|\bultima\s+semana\b/.test(n)) return "semana";
  if (/\bmes\b|\bmes\s+passado\b|\beste\s+mes\b/.test(n)) return "mes";
  return undefined;
}

/** Extrai tipo de relatório para PDF: estoque, vendas, clientes, produtos. */
export function extrairTipoRelatorioPdf(texto: string): EntidadesExtraidas["tipoRelatorio"] {
  const n = normalizarTexto(texto);
...
export function extrairData(texto: string): string | undefined {
  const n = normalizarTexto(texto);
...
export function extrairCnpj(texto: string): string | undefined {
 ...
 */
export function extrairEntidades(pergunta: string): EntidadesExtraidas {
...
export const PALAVRAS_CHAVE: Record<string, RegExp[]> = {
  pedido_por_numero: [
 ...
 */
export function identificarIntencao(pergunta: string): {
  intencao: string;
  ...
  for (const [intencao, regexes] of Object.entries(PALAVRAS_CHAVE)) {
    if (regexes.some((r) => r.test(n))) {
      if (intencao.startsWith("relatorio_pdf_")) return { intencao, entidades };
      ...
      if ((intencao === "cliente_devedor" || intencao === "boletos_cliente") && !entidades.nomeCliente)
        (entidades as EntidadesExtraidas & { nomeCliente?: string }).nomeCliente = extrairNomeCliente(pergunta) ?? undefined;
      if (intencao === "dar_baixa_pedido" && entidades.numeroPedido == null)
        (entidades as EntidadesExtraidas & { numeroPedido?: number }).numeroPedido = extrairNumeroPedido(pergunta) ?? undefined;
      if ((intencao === "consultar_cep") && !entidades.cep) (entidades as EntidadesExtraidas & { cep?: string }).cep = extrairCep(pergunta);
      if ((intencao === "consultar_cnpj") && !entidades.cnpj) (entidades as EntidadesExtraidas & { cnpj?: string }).cnpj = extrairCnpj(pergunta);
      if ((intencao === "consultar_clima") && !entidades.cidade) (entidades as EntidadesExtraidas & { cidade?: string }).cidade = extrairCidade(pergunta);
      if ((intencao === "rastrear_pedido" || intencao === "rastrear_entrega") && !entidades.codigoRastreio) (entidades as EntidadesExtraidas & { codigoRastreio?: string }).codigoRastreio = extrairCodigoRastreio(pergunta);
      if (intencao === "ultima_carga_cidade" && !entidades.cidade) {
        ...
        if (m && m[1]) (entidades as EntidadesExtraidas & { cidade?: string }).cidade = m[1].trim().slice(0, 80);
      }
      return { intencao, entidades };
    }
  }

  if (entidades.numeroPedido != null) return { intencao: "pedido_por_numero", entidades };
