...
function buildBaseConditions(tenantId: number, range?: MetricsDateRange) {
  ...
  if (range?.startDate) {
    conditions.push(gte(auditLogs.createdAt, range.startDate));
  }

  if (range?.endDate) {
    conditions.push(lte(auditLogs.createdAt, range.endDate));
  }
...
export class MetricsService {
  static async getTotalLogs(tenantId: number, range?: MetricsDateRange): Promise<Payload> {
  ...
  static async getLogsByAction(tenantId: number, range?: MetricsDateRange): Promise<Payload> {
    const db = await getDb();
    const conditions = buildBaseConditions(tenantId, range);
  ...
  static async getLogsByEntity(tenantId: number, range?: MetricsDateRange): Promise<Payload> {
    const db = await getDb();
    const conditions = buildBaseConditions(tenantId, range);
    const countExpr = sql<number>`COUNT(*)`;
  ...
  static async getErrorRate(tenantId: number, range?: MetricsDateRange): Promise<Payload> {
    const db = await getDb();
    ...
    const ipCountExpr = sql<number>`COUNT(*)`;
  ...
  static async getLogsPerDay(tenantId: number, range?: MetricsDateRange): Promise<Payload> {
    const db = await getDb();
    const conditions = buildBaseConditions(tenantId, range);
    const dateExpr = sql<string>`DATE(${auditLogs.createdAt})`;
    const countExpr = sql<number>`COUNT(*)`;
