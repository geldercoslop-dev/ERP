  ...
  for (const [key, value] of Object.entries(payload)) {
    ...
    if (isString(value)) {
      ...
      sanitized[key] = limitedArray.map(item => 
        isRecord(item) ? sanitizePayload(item as Record<string, unknown>) : 
 ...
 */
function extractIP(request?: { ip?: string; headers?: Record<string, string>; socket?: { remoteAddress?: string } }): string | null {
 ...
 */
function extractUserAgent(request?: { headers?: Record<string, string> }): string | null {
 ...
 */
function validateAuditPayload(payload: AuditLogPayload): { success: boolean; error?: string } {
  if (!isRecord(payload)) {
...
export class AuditLogService {
   ...
   */
  static async logAction(payload: AuditLogPayload, request?: { ip?: string; headers?: Record<string, string>; socket?: { remoteAddress?: string } }): Promise<{ success: boolean; error?: string }> {
    try {
      // Validação
  ...
  static async getAuditLogs(
    ...
    try {
      ...
      if (filters.ip) conditions.push(eq(auditLogs.ip, filters.ip));
   ...
   */
  static async cleanupOldLogs(retentionDays: number = 90): Promise<{ success: boolean; data?: number; error?: string }> {
    try {
      const db = await getDb();
