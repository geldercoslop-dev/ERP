...
import { ADMIN_ACTOR, type ServiceActor } from "../_core/service-actor";

export * from "./core";

export { getConnectionPool } from "../config/database";

export { gerarBackupCompleto } from "../services/backup.service";

/** Alias para compatibilidade */
export type { NewUser as InsertUser, NewVendedor as InsertVendedor } from "./core";
...
export async function getCargaById(
  tenantId: number,
  id: number
): Promise<import("../services/logistica.service").CargaComPedidos | null> {
  const logistica = await import("../services/logistica.service");
  return logistica.getCargaById(tenantId, id);
}

export { getPedidoById } from "./core";
...
export async function getAllProdutosComPrecoVigente(tenantId: number, refDate: Date = new Date()) {
  const inv = await import("../services/inventory.service");
  return inv.getAllProdutosComPrecoVigente(tenantId, refDate);
}

export async function getProdutosComPrecoVigentePaged(
  tenantId: number,
  opts: import("../services/inventory.service").GetProdutosComPrecoVigentePagedOpts
) {
  const inv = await import("../services/inventory.service");
...
export async function getAllClientes(tenantId: number) {
  const cli = await import("../services/clientes.service");
...
export async function updateCliente(tenantId: number, id: number, patch: unknown) {
  const cli = await import("../services/clientes.service");
  return cli.updateCliente(tenantId, id, patch as never);
}

export async function deleteClienteById(tenantId: number, id: number) {
  const cli = await import("../services/clientes.service");
  return cli.deleteCliente(tenantId, id);
}
...
export async function updatePedido(tenantId: number, id: number, data: unknown, itens?: unknown) {
  const orders = await import("../services/orders.service");
  // itens é tratado no router; aqui delegamos apenas update do pedido
  void itens;
  return orders.updatePedido(tenantId, id, data as never);
}

export async function deletePedido(tenantId: number, id: number) {
  const orders = await import("../services/orders.service");
  return orders.deletePedido(tenantId, id);
}
...
export async function baixarPedidoDireto(
  ...
  pedidoId: number,
  data: unknown,
  actor?: import("../_core/service-actor").ServiceActor
) {
  const finance = await import("../services/finance.service");
...
export async function listPendencias(tenantId: number, vendedorId?: number) {
  const pend = await import("../services/pendencias.service");
...
export async function getBoletosByVendedor(tenantId: number, vendedorId: number) {
  const fin = await import("../services/finance.service");
...
function parseContaReceberPayload(input: unknown): import("../services/finance.service").CreateContaReceberInput {
...
export async function listContasPagarFiltro(tenantId: number, input: unknown) {
  const fin = await import("../services/finance.service");
  const { ADMIN_ACTOR } = await import("../_core/service-actor");
...
export async function listPromocoes(tenantId: number, options?: { page?: number; pageSize?: number }) {
  const promo = await import("../services/promocoes.service");
...
export async function runDiagnosticoConsistencia(tenantId: number) {
  const sys = await import("../services/system.service");
