...
function isStockDomainError(e: unknown): e is { code?: string; message?: string } {
...
export async function buscarProdutos(request: Request) {
  ...
  try {
    ...
    const filtrados = (produtos as ProdutoBuscaItem[]).filter((p) =>
...
export async function verificarEstoqueBaixo(_req: Request) {
  ...
  try {
    ...
    const baixo = (produtos as ProdutoEstoqueItem[]).filter((p) => Number(p.estoque ?? 0) <= 0);
