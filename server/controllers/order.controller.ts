...
class OrderController {
  ...
  static async create(req: RequestWithTenant, res: Response): Promise<void> {
    ...
    try {
      ...
      const createOrderData: CreateOrderData = {
        tenantId,
        clienteNome: payload.clienteNome as string,
        formaPagamento: payload.formaPagamento as string,
        itens: payload.itens as OrderItem[]
      };
      ...
      if ('boletoVencimentos' in payload && Array.isArray(payload.boletoVencimentos)) {
        createOrderData.boletoVencimentos = payload.boletoVencimentos as Date[];
      }
  ...
  static async list(req: RequestWithTenant, res: Response): Promise<void> {
  ...
  static async getById(req: RequestWithTenant, res: Response): Promise<void> {
    ...
    try {
      ...
      const order = Array.isArray(result) ? result.find((o: unknown) => typeof o === 'object' && o !== null && 'id' in o ? (o as { id: number }).id === id : false) : null;
  ...
  static async updateStatus(req: RequestWithTenant, res: Response): Promise<void> {
    try {
      const id = parseInt(req.params?.id as string);
      if (!id || id <= 0) {
        res.status(400).json({
      ...
      const result = await updateOrderStatusSafe(
        tenantId,
        id,
        status,
        typeof req.body?.motivo === 'string' ? req.body.motivo : 'Status update',
        typeof req.body?.userId === 'number' ? req.body.userId : undefined
      );
      res.status(200).json({
        success: true,
        data: result,
