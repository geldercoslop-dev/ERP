 ...
 * - Taxa de erros
 */
export async function getInternalStatus(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    logger.error('Error generating internal status', error as Error, {
      metadata: {
 ...
 */
export function internalStatusAuthGuard(
  req: Request,
  res: Response,
  next: (err?: Error) => void
): void {
 ...
 */
export function getPublicHealth(req: Request, res: Response): void {
  const memUsage = process.memoryUsage();
